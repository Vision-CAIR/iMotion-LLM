        ...
        for data_file in self.data_files:
            ...
            for data in dataset:
                ...
                for pairs_idx, pairs in enumerate(self.sdc_ids_list):
                    ...
                    # t_ = time.time()
                    # agent_json = self.gen_instruct_caption_01(history=np.vstack((deepcopy(ego),deepcopy(neighbors))),future=deepcopy(ground_truth), navigation_extractor=navigation_extractor, vizualize=not args.not_debug, viz_dir= 'ex.png' if not args.not_debug else viz_dir)
                    ...
                    if args.plausbility:
                        ...
                        # print('>Start')
                        # t_ = time.time()
                        plausibility = self.get_direction_plausibility_02(deepcopy(ego), deepcopy(map_lanes), deepcopy(map_crosswalks), deepcopy(ground_truth), deepcopy(additional_boundaries_), deepcopy(traffic_lights), deepcopy(stop_signs), deepcopy(agent_json_))
                        # print(plausibility_)
                        # print(f'#End {time.time()-t_:.2f}s')
                        # continue
                        # print(plausibility)

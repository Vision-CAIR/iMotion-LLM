...
def vizualize_background(segment, segment_type, fig, categ, ax, force_color=None):
    ...
    if categ!=3 and categ!=6:
        if force_color is not None:
            plt.plot(segment[:,0], segment[:,1], (force_color,'solid', 1)[0], linestyle=(force_color,'solid', 1)[1], linewidth=2, alpha=(force_color,'solid', 1)[2], zorder=z_orders[categ])
        else:
            plt.plot(segment[:,0], segment[:,1], plt_info[categ][segment_type][0], linestyle=plt_info[categ][segment_type][1], linewidth=2, alpha=plt_info[categ][segment_type][2], zorder=z_orders[categ])
    elif categ==3:
...
class DataProcess(object):
    ...
    def get_direction_plausibility_03(self, ego_lane, drivable_lanes, ego, map_lanes, map_crosswalks, ground_truth, additional_boundaries_, traffic_lights, stop_signs, agent_json_):
    ...
    def process_data(self, viz=True,test=False):
        ...
        for iii, data_file in enumerate(self.data_files):
            ...
            for data in dataset:
                ...
                #     continue
                if scenario_id not in ['4d54f48a3e3662e4', '4d54f48a3e3662e4', '4d54f48a3e3662e4', '4d54f48a3e3662e4', '4d54f48a3e3662e4', '4d54f48a3e3662e4', '4d54f48a3e3662e4', '4d54f48a3e3662e4', 'adff7d3a18bbb9b0', 'adff7d3a18bbb9b0', '36c9fb88a5641e5', '36c9fb88a5641e5', 'c59ddfe50c3fb534', 'c59ddfe50c3fb534', 'c59ddfe50c3fb534', 'c59ddfe50c3fb534', 'c59ddfe50c3fb534', '6dd82b87407da0a5', '6dd82b87407da0a5', '6dd82b87407da0a5', '6dd82b87407da0a5', '6dd82b87407da0a5', '6dd82b87407da0a5', '6dd82b87407da0a5', '6dd82b87407da0a5', '9f1e8b4a23eeab0', '9f1e8b4a23eeab0', '9f1e8b4a23eeab0', '9f1e8b4a23eeab0', '33f5ce0750e2a620', 'e9e3a8360ac580e8', 'e9e3a8360ac580e8', 'e9e3a8360ac580e8', 'e9e3a8360ac580e8', '50d7785dd832d9d5', '14eaa29b601ff21e']:
                ...
                for pairs_idx, pairs in enumerate(self.sdc_ids_list):
                    ...
                    # t_ = time.time()
                    # if valid_instruct:    
                    ego, neighbors, map_lanes, map_crosswalks, ground_truth, region_dict, normalization_param, speed_bumps = self.normalize_data(deepcopy(ego), deepcopy(neighbors), deepcopy(map_lanes[np.newaxis]), deepcopy(map_crosswalks[np.newaxis]), deepcopy(ground_truth), deepcopy(speed_bumps[np.newaxis]), viz=debug)
                    ...
                    if valid_instruct and args.plausbility:
                        ...
                        # print('>Start')
                        # t_ = time.time()
                        # plausibility = self.get_direction_plausibility(ego, map_lanes, map_crosswalks, ground_truth, additional_boundaries_, traffic_lights, stop_signs, agent_json_)
                        # print(plausibility)
                        # print(f'#End {time.time()-t_:.2f}s')
                        # print('***'*10)
                        # print("P2:")
                        # print('>Start')
                        # t_ = time.time()
                        if drivable_lanes_new is not None:
                            plausibility, directions_lanes, _ = self.get_direction_plausibility_03(deepcopy(ego_lane), deepcopy(drivable_lanes_new), deepcopy(ego), deepcopy(map_lanes), deepcopy(map_crosswalks), deepcopy(ground_truth), deepcopy(additional_boundaries_), deepcopy(traffic_lights), deepcopy(stop_signs), deepcopy(agent_json_))
                        else:
                        ...
                        # continue
                        # print(plausibility)
                        # plausibility2 = self.get_direction_plausibility_vectorized(ego, map_lanes, map_crosswalks, ground_truth, additional_boundaries_, traffic_lights, stop_signs, agent_json_)
                        # print(plausibility2)
                        
                        # if time.time()-t_ > 10:
                        #     self.get_direction_plausibility(ego, map_lanes, map_crosswalks, ground_truth, additional_boundaries_, traffic_lights, stop_signs, agent_json_)
                        # continue
                        ...
                        if vizualize:
                            ...
                            subsample_indices = np.linspace(0, 80 - 1, 8, dtype=int) # subsample to sample/second
                            # fig = self.plt_scene((history_global_frame[0], future_global_frame[0, subsample_indices]),(history_global_frame[1], future_global_frame[1, subsample_indices]), history_global_frame[2:])
                            # fig = self.plt_scene((history_viz[0], future_viz[0, subsample_indices]), max_num_lanes=499, direction_lanes=directions_lanes, map_lanes = deepcopy(map_lanes))
                            # fig.savefig('ex.png')
                            # plt.close
                            # fig2 = self.plt_scene((history_viz[0], future_viz[0, subsample_indices]), max_num_lanes=499, direction_lanes=directions_lanes, map_lanes = deepcopy(drivable_lanes))
                            # fig2.savefig('ex2.png')
                            # plt.close()
                            # fig3 = self.plt_scene((history_viz[0], future_viz[0, subsample_indices]), max_num_lanes=1000, map_lanes = deepcopy(drivable_lanes), drivable_lanes=drivable_lanes_new)
                            # fig3 = self.plt_scene((history_viz[0], future_viz[0, subsample_indices]), max_num_lanes=500, map_lanes = deepcopy(map_lanes), drivable_lanes=deepcopy(drivable_lanes_new))
                            ...
                            # fig3 = self.plt_scene((history_viz[0], future_viz[0, subsample_indices]), max_num_lanes=500, map_lanes = deepcopy(map_lanes), drivable_lanes=deepcopy(drivable_lanes_new))
                            # fig3 = self.plt_scene((history_viz[0], future_viz[0, subsample_indices]), max_num_lanes=1000, drivable_lanes=drivable_lanes_new)
                            # fig3 = self.plt_scene(max_num_lanes=499, drivable_lanes=drivable_lanes_new)
                            ...
                            # fig3 = self.plt_scene((history_viz[0], future_viz[0, subsample_indices]), max_num_lanes=500, map_lanes = deepcopy(map_lanes), drivable_lanes=drivable_lanes_new)
                            # fig3 = self.plt_scene((history_viz[0], future_viz[0, subsample_indices]), max_num_lanes=1000, map_lanes = deepcopy(map_lanes), drivable_lanes=drivable_lanes_new)
                            # fig3 = self.plt_scene((history_viz[0], future_viz[0, subsample_indices]), max_num_lanes=1000, drivable_lanes=drivable_lanes_new)
                    ...
                    if True:
                        if test:
                            ...
                            if valid_instruct:
                                ...
                                # print('')
                                # with open(acts_filename, 'rb') as f:
                                #     loaded_data = pickle.load(f)
                                # eval(loaded_data)
                            # with open(templateLLM_filename, 'r') as file:
                            #     content = file.read()
                            ...
                            if not debug:
                                np.savez_compressed(filename, ego=np.array(original_ego), neighbors=np.array(original_neighbors), map_lanes=np.array(original_map_lanes), 
                                    ...
                                    # additional data
                                    # additional_map_lanes=np.array(map_lanes)[:n_lanes, :t_samples:t_subsampling, :], 
                                    # additional_map_crosswalks=np.array(map_crosswalks)[:, :100:t_subsampling],
                                    # additional_boundaries=np.array(additional_boundaries_)[:n_lanes, :t_samples:t_subsampling, :],
                                    # traffic_lights=np.array(traffic_lights),
                                    # stop_signs=np.array(stop_signs),
                                    # speed_bumps=np.array(speed_bumps)[:, :100:t_subsampling]
                                    )
                            # np.savez(filename, ego=np.array(ego), neighbors=np.array(neighbors), map_lanes=np.array(map_lanes), 
                            # map_crosswalks=np.array(map_crosswalks),object_type=np.array(object_type),region_6=np.array(region_dict[6]),
                            # object_index=np.array(object_index),current_state=np.array(self.current_xyzh[0]),gt_future_states=np.array(ground_truth),
                            # boundaries_L=np.array(map_extra_boundaries_L), boundaries_R=np.array(map_extra_boundaries_R), additional_boundaries=np.array(additional_boundaries_))
                            if not debug:
                    ...
                    # else:
                        ...
                        #     'boundaries_L': None,
                        #     'boundaries_R': None,
                        #     'additional_boundaries': torch.tensor(additional_boundaries_).unsqueeze(0),
                        # }
                        # ego_future = torch.tensor(ground_truth[0]).unsqueeze(0)
                        # # t_ = time.time()
                        # viz_multimodal(inputs, ego_future.unsqueeze(0)[...,:2], ego_future).savefig('ex_v5.png')

                        # #     'ego_state': torch.tensor(ego[0:1]), # [1, 11 history steps,11 features]
                        # #     'neighbors_state': torch.cat((torch.tensor(ego[1:]),torch.tensor(neighbors)), dim=0).unsqueeze(0), # [1, 21 agents (excluding the ego), 11 history steps,11 features]
                        # #     'map_lanes': torch.tensor(map_lanes).unsqueeze(0), # [1, 2 interactive agents (the ego and the first of the neighbors_state), 6 drivable lanes to be increased, 300 samples, 17 features]
                        # #     'map_crosswalks': torch.tensor(map_crosswalks).unsqueeze(0), # [1, 2 agents, 4 closest crosswalks, 100 samples, 3 features]
                        # #     'boundaries_L': torch.tensor(map_extra_boundaries_L).unsqueeze(0),
                        # #     'boundaries_R': torch.tensor(map_extra_boundaries_R).unsqueeze(0),
                        # #     'additional_boundaries': None,
                        # # }
                        # # ego_future = torch.tensor(ground_truth[0]).unsqueeze(0)
                        # # # t_ = time.time()
                        # # viz_multimodal(inputs, ego_future.unsqueeze(0)[...,:2], ego_future).savefig('ex_v5_noaddboundaries.png')
                        ...
                        #     'additional_boundaries': None,
                        # }
                        # viz_multimodal(inputs, ego_future.unsqueeze(0)[...,:2], ego_future).savefig('ex_6lanes.png')
                        ...
                        #     'additional_boundaries': None,
                        # }
                        # viz_multimodal(inputs, ego_future.unsqueeze(0)[...,:2], ego_future).savefig('ex_100lanes.png')
                        
                        # ego
                        # neighbors
                        # map_lanes
                        # map_crosswalks, object_type
                        # gt_future_states
                        ...
                        # with open(map_json_filename, 'w') as file:
                        #     json.dump(map_json, file, indent=4)  #

                        # print('')
                        
                        # print(f"Vizualization: {time.time()-t_}")
                        # print("")
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--load_path', type=str, help='path to dataset files', default='/ibex/project/c2278/felembaa/datasets/waymo/validation_interactive')
    parser.add_argument('--save_path', type=str, help='path to save processed data', default = '/ibex/project/c2278/temp/temp')

...
class DataProcess(object):
    ...
    def process_data(self, viz=True,test=False):
        templatellm_filename_map = {}
        acts_filename_map = {}
        np_filename_map = {}
        agent_json_filename_map = {}
        map_json_filename_map = {}
        ...
        for iii, data_file in enumerate(self.data_files):
            ...
            for data in dataset:
                ...
                # if scenario_id not in ['ae62c469dfea9db3']:
                # # if '6a6f89c71d15f90d' not in scenario_id:
                #     continue
                ...
                for pairs_idx, pairs in enumerate(self.sdc_ids_list):
                    ...
                    mtr_id = {v:k for k,v in id_list.items()}[pairs[0][0]]
                    mtr_id = f"{scenario_id}_{mtr_id}"
                    # print(f">>>>>>>>>>>. MTR ID (scenario_id & center_object_id): {mtr_id}")
                    ...
                    if valid_instruct and args.plausbility:
                        ...
                        if vizualize:
                            ...
                            fig3 = self.plt_scene((history_viz[0], future_viz[0, subsample_indices]), max_num_lanes=500, map_lanes = deepcopy(map_lanes), drivable_lanes=deepcopy(drivable_lanes_new), direction_lanes=directions_lanes)

                            fig3.savefig('ex3.png')
                            plt.close()
                            
                            self.get_drivable_lanes(ego[0])
                            self.get_direction_plausibility_03(deepcopy(ego_lane), deepcopy(drivable_lanes_new), deepcopy(ego), deepcopy(map_lanes), deepcopy(map_crosswalks), deepcopy(ground_truth), deepcopy(additional_boundaries_), deepcopy(traffic_lights), deepcopy(stop_signs), deepcopy(agent_json_))

                    else:
                        continue
                        possible_directions = []
                        possible_directions_cls = []
                        not_possible_directions = []
                        not_possible_directions_cls = []
                    ...
                    if True:
                        if test:
                            ...
                            # TODO: save mtrid_npfilename
                            mtr_id = {v:k for k,v in id_list.items()}[pairs[0][0]]
                            mtr_id = f"{scenario_id}_{mtr_id}"
                            print(f">>>>>>>>>>>. MTR ID: {mtr_id}")
                            print('')
                            ...
                            if valid_instruct:
                                ...
                                templatellm_filename_map[mtr_id] = templateLLM_filename
                            ...
                            if not debug:
                                ...
                                #     )
                            if not debug:
                                if valid_instruct and False:
                                    agent_json_filename_map[mtr_id] = agent_json_filename
                                    # with open(agent_json_filename, 'w') as file:
                                    #     json.dump(agent_json, file, indent=4)  # indent=4 for pretty printing
                                    map_json_filename_map[mtr_id] = map_json_filename
                        ...
                        if debug:
                            ...
                            inputs = {
                                ...
                                'neighbors_state': torch.cat((torch.tensor(ego[1:]),torch.tensor(neighbors[:1])), dim=0).unsqueeze(0), # [1, 21 agents (excluding the ego), 11 history steps,11 features]
                                'map_lanes': torch.tensor(map_lanes)[...,:n_lanes,:t_samples:t_subsampling,:].unsqueeze(0), # [1, 2 interactive agents (the ego and the first of the neighbors_state), 6 drivable lanes to be increased, 300 samples, 17 features]
                                'map_crosswalks': torch.tensor(map_crosswalks).unsqueeze(0), # [1, 2 agents, 4 closest crosswalks, 100 samples, 3 features]
                                'boundaries_L': None,
                                'boundaries_R': None,
                                'additional_boundaries': torch.tensor(additional_boundaries_)[...,:n_lanes,:t_samples:t_subsampling,:].unsqueeze(0),
                            }
                            ego_future = torch.tensor(ground_truth[0]).unsqueeze(0)
                            # t_ = time.time()
                            viz_multimodal(inputs, ego_future.unsqueeze(0)[...,:2], ego_future).savefig('ex_100lanes_withboundaries.png')
                            if valid_instruct:
        ...
        os.makedirs(root_save_dir+'/gf_mtr_mapping', exist_ok=True)
        self.templatellm_filename_map_dir = root_save_dir + '/gf_mtr_mapping/gf_templatellm_maps'
        self.acts_filename_map_dir = root_save_dir + '/gf_mtr_mapping/gf_acts_maps'
        self.np_filename_map_dir = root_save_dir + '/gf_mtr_mapping/gf_np_maps'
        self.agent_json_filename_map_dir = root_save_dir + '/gf_mtr_mapping/gf_agent_json_maps'
        self.map_json_filename_map_dir = root_save_dir + '/gf_mtr_mapping/gf_map_json_maps'
        # Create directories if they do not exist
        os.makedirs(self.templatellm_filename_map_dir, exist_ok=True)
        os.makedirs(self.acts_filename_map_dir, exist_ok=True)
        os.makedirs(self.np_filename_map_dir, exist_ok=True)
        os.makedirs(self.agent_json_filename_map_dir, exist_ok=True)
        os.makedirs(self.map_json_filename_map_dir, exist_ok=True)
        # Generate a unique filename for each process
        unique_id = str(uuid.uuid4())
        # Save JSON files
        with open(os.path.join(self.templatellm_filename_map_dir, f"{unique_id}.json"), 'w') as file:
            json.dump(templatellm_filename_map, file, indent=4)
        with open(os.path.join(self.acts_filename_map_dir, f"{unique_id}.json"), 'w') as file:
            json.dump(acts_filename_map, file, indent=4)
        with open(os.path.join(self.np_filename_map_dir, f"{unique_id}.json"), 'w') as file:
            json.dump(np_filename_map, file, indent=4)
        with open(os.path.join(self.agent_json_filename_map_dir, f"{unique_id}.json"), 'w') as file:
            json.dump(agent_json_filename_map, file, indent=4)
        with open(os.path.join(self.map_json_filename_map_dir, f"{unique_id}.json"), 'w') as file:
            json.dump(map_json_filename_map, file, indent=4)
        ## Save the following jsons:
        # templatellm_filename_map = {}
        # acts_filename_map = {}
        # np_filename_map = {}
        # agent_json_filename_map = {}

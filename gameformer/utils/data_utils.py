...
from gameformer.utils.cubic_spline_planner import Spline2D
import torch
...
def find_reference_lanes_all_lanes(agent_type, agent_traj, lanes, max_num_lanes=10, max_distance_threshold=50):
    curr_lane_ids = {}
    curr_lane_distances = {}
    
        
    if agent_type == 2:
        distance_threshold_ = 5
        distance_threshold = distance_threshold_

        while len(curr_lane_ids) < max_num_lanes:
        ...
        # distance_threshold_ = 3.5
        distance_threshold_ = 50
        distance_threshold = distance_threshold_
        ...
        for lane in lanes.keys():
            if lanes[lane].shape[0] > 1:
                ...
                distance_to_agent = np.linalg.norm(agent_traj[-1, :2] - lanes[lane][:, :2], axis=-1)
            if distance_to_agent < distance_threshold:
                curr_lane_ids[lane] = 0 
                curr_lane_distances[lane] = distance_to_agent
    ...
    # all_lane_ids = {} # all lane ids without filtering anything out
    # for lane in lanes.keys():
    #     for k in range(distance_to_ego.shape[0]):
    #         all_lane_ids[lane] = k
    # return curr_lane_ids, all_lane_ids
    sorted_indices = [index for index, value in sorted(enumerate(list(curr_lane_distances.values())), key=lambda x: x[1])]
    lanes_numbers = list(curr_lane_ids.keys())
    curr_lane_ids = {lanes_numbers[idx]: curr_lane_ids[lanes_numbers[idx]] for idx in sorted_indices} #sorted based on distance
    # {lanes_numbers[idx]: curr_lane_distances[lanes_numbers[idx]] for idx in sorted_indices} 
...
def find_reference_lanes_new(agent_type, agent_traj, lanes, max_num_lanes=1, max_distance_threshold=50):
    curr_lane_ids = {}
    ...
    if agent_type == 2:
        ...
        distance_threshold_ = 3.5
        # distance_threshold_ = 100
        distance_threshold = distance_threshold_
        # direction_threshold = 10
        while len(curr_lane_ids) < max_num_lanes:
            for lane in lanes.keys(): # iterate over all lanes
                distance_to_ego = np.linalg.norm(agent_traj[-1, :2] - lanes[lane][:, :2], axis=-1)
                direction_to_ego = compute_direction_diff(agent_traj[-1, 2], lanes[lane][:, -1])
                for i, j, k in zip(distance_to_ego, direction_to_ego, range(distance_to_ego.shape[0])):    
                    if i <= distance_threshold:# and j <= np.radians(direction_threshold):
...
def find_all_lanes(agent_type, agent_traj, lanes, max_num_lanes=1, max_distance_threshold=300):
    distance_threshold = max_distance_threshold
    curr_lane_ids = {}
    curr_lane_min_distance = {}
    
    for lane in lanes.keys(): # iterate over all lanes
        # distance_to_ego = np.linalg.norm(agent_traj[-1, :2] - lanes[lane][:, :2], axis=-1)
        # direction_to_ego = compute_direction_diff(agent_traj[-1, 2], lanes[lane][:, -1])
        # curr_lane_ids[lane] = distance_to_ego.argmin()
        curr_lane_ids[lane] = 0
        curr_lane_min_distance[lane] = np.linalg.norm(agent_traj[-1, :2] - lanes[lane][:, :2], axis=-1).min()
    
    sorted_curr_lane_ids = {k:curr_lane_ids[k] for k, v in sorted(curr_lane_min_distance.items(), key=lambda item: item[1])}
    return sorted_curr_lane_ids

def find_neighbor_lanes_fullmap(curr_lane_ids, traj, lanes, lane_polylines):
    neighbor_lane_ids = {}
    ...
    for curr_lane, start in curr_lane_ids.items():
        ...
        for l_lane in left_lanes:
            if l_lane.feature_id not in curr_lane_ids.keys():
                left_lane = l_lane


        for r_lane in right_lanes:
            if r_lane.feature_id not in curr_lane_ids.keys():
                right_lane = r_lane
...
def map_norm(map_line, center, angle):
    # Check if map_line has only one point
    if len(map_line) == 1:
        # Temporarily duplicate the point to make it work with LineString operations
        map_line = np.vstack([map_line, map_line])
        unstack_map_line = True
    else:
        unstack_map_line = False
    ...
    if map_line.shape[1] >2:
        self_heading = wrap_to_pi(map_line[:, 2] - angle)
    else:
        self_heading = np.array([0]*len(map_line))
    ...
    if map_line.shape[1] > 3:
        ...
        if unstack_map_line:
            return np.column_stack((self_line, self_heading))[0:1]
        else:
...
def reverse_agent_norm(norm_traj, center, angle):
    # First, reverse the rotation
    reversed_rotation = np.array([rotate(Point(x, y), angle, origin=(0, 0), use_radians=True) for x, y in norm_traj[:, :2]])
    
    # Then reverse the translation
    reversed_translation = np.array([(p.x + center[0], p.y + center[1]) for p in reversed_rotation])
    
    # Adjust heading back
    heading = wrap_to_pi(norm_traj[:, 2] + angle)
    
    # Reverse velocities if present
    if norm_traj.shape[1] > 3:
        velocity_x = norm_traj[:, 3] * np.cos(-angle) + norm_traj[:, 4] * np.sin(-angle)
        velocity_y = -norm_traj[:, 3] * np.sin(-angle) + norm_traj[:, 4] * np.cos(-angle)
        return np.column_stack((reversed_translation, heading, velocity_x, velocity_y))
    else:
        return np.column_stack((reversed_translation, heading))

def reverse_map_norm(norm_map_line, center, angle):
    # Reverse the main line points
    main_line = np.array([rotate(Point(x, y), angle, origin=(0, 0), use_radians=True) for x, y in norm_map_line[:, :2]])
    main_line_translated = np.array([(p.x + center[0], p.y + center[1]) for p in main_line])
    main_heading = wrap_to_pi(norm_map_line[:, 2] + angle)

    result = np.column_stack((main_line_translated, main_heading))

    # Check if there are additional coordinates for left and right lines
    if norm_map_line.shape[1] > 3:
        # Process additional lines similarly
        left_line = np.array([rotate(Point(x, y), angle, origin=(0, 0), use_radians=True) for x, y in norm_map_line[:, 3:5]])
        left_line_translated = np.array([(p.x + center[0], p.y + center[1]) for p in left_line])
        left_heading = wrap_to_pi(norm_map_line[:, 5] + angle)

        right_line = np.array([rotate(Point(x, y), angle, origin=(0, 0), use_radians=True) for x, y in norm_map_line[:, 6:8]])
        right_line_translated = np.array([(p.x + center[0], p.y + center[1]) for p in right_line])
        right_heading = wrap_to_pi(norm_map_line[:, 8] + angle)

        result = np.column_stack((result, left_line_translated, left_heading, right_line_translated, right_heading))

    return result

def reverse_ref_line_norm(norm_ref_line, center, angle):
    # Reverse rotation and translation
    reversed_xy = np.array([rotate(Point(x, y), angle, origin=(0, 0), use_radians=True) for x, y in norm_ref_line[:, :2]])
    reversed_translation = np.array([(p.x + center[0], p.y + center[1]) for p in reversed_xy])
    
    # Reverse the yaw angles
    yaw = wrap_to_pi(norm_ref_line[:, 2] + angle)
    
    # Curvature and additional info remain unchanged
    c = norm_ref_line[:, 3]
    info = norm_ref_line[:, 4]
    
    return np.column_stack((reversed_translation, yaw, c, info))

def reverse_normalize_traj_to_global_map(ego, neighbors, ground_truth, normalization_param):

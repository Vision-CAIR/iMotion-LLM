...
def map_process(anchor_state, coords, traffic_light_data, map_features, max_elements, max_points, interpolation_method, join_lanes=False):
    """
    ...
    """
    if True in [list_tensor_data[f"traffic_light_data.LANE"][i]==[0,0,0,1] for i in range(len(list_tensor_data[f"traffic_light_data.LANE"]))]:
        print('')
    tensor_output = {}
        ...
        if f"coords.{feature_name}" in list_tensor_data:
            ...
            if join_lanes:
                if feature_coords is not None and len(feature_coords)>0:
                    feature_coords = [torch.cat(feature_coords, 0)]
                if feature_tl_data is not None:
                    feature_tl_data = feature_tl_data[0:1] # using the tl info from the first segment only
    ...
    for feature_name in map_features:
        if feature_name == "LANE":
            ...
            polylines = tensor_output[f'vector_set_map.coords.{feature_name}'].numpy()
            avails = tensor_output[f'vector_set_map.availabilities.{feature_name}'].numpy()
            vector_map_route_lanes_L = polyline_process(polylines, avails)
        elif feature_name == "RIGHT_BOUNDARY":
            polylines = tensor_output[f'vector_set_map.coords.{feature_name}'].numpy()
            avails = tensor_output[f'vector_set_map.availabilities.{feature_name}'].numpy()
            vector_map_route_lanes_R = polyline_process(polylines, avails)
        else:
            pass
    
    
    vector_map_crosswalks = None if "CROSSWALK" not in map_features else vector_map_crosswalks
    vector_map_route_lanes = None if "ROUTE_LANES" not in map_features else vector_map_route_lanes
    vector_map_output = {'lanes': vector_map_lanes, 'crosswalks': vector_map_crosswalks, 'route_lanes': vector_map_route_lanes, 
    'left_boundary': vector_map_route_lanes_L, 'right_boundary': vector_map_route_lanes_R}
...
def create_ego_raster(vehicle_state):
    ...
    # Paint the rectangle
    rect = plt.Rectangle(ego_bottom_right, ego_front_length+ego_rear_length, ego_width, linewidth=2, color='b', alpha=0.8, zorder=3,
                        transform=mpl.transforms.Affine2D().rotate_around(*(x_center, y_center), heading) + plt.gca().transData)
    plt.gca().add_patch(rect)


def create_agents_raster(agents, agent_colors=None):
    if agent_colors is None:
        agent_colors=['m']*len(agents)
    for i in range(agents.shape[0]):
        if agents[i, 0] != 0:
            x_center, y_center, heading = agents[i, 0], agents[i, 1], agents[i, 2]
            # agent_length, agent_width = agents[i, 6],  agents[i, 7]
            agent_length, agent_width = agents[i, 5],  agents[i, 6]
            agent_bottom_right = (x_center - agent_length/2, y_center - agent_width/2)
            
            rect = plt.Rectangle(agent_bottom_right, agent_length, agent_width, linewidth=2, color=agent_colors[i], alpha=0.8, zorder=3,
                                transform=mpl.transforms.Affine2D().rotate_around(*(x_center, y_center), heading) + plt.gca().transData)
            plt.gca().add_patch(rect)


def create_map_raster(lanes, crosswalks=None, route_lanes=None, right_boundaries=None, left_boundaries=None, special_lanes=None, num_lanes=-1):
    lanes = lanes[...,:num_lanes,:,:]
    tl_colors = {6: 'green', 5: 'yellow', 4: 'red'}
    if len(lanes.shape)==4:
        lanes = lanes[0]
    for i in range(lanes.shape[0]):
        lane = lanes[i][np.linalg.norm(lanes[i][:, :2], axis=-1)>0]
        if len(lane)==0:
            continue
        lane_color = 'gray'
        # if lane[0][0] != 0:
        plt.plot(lane[:, 0], lane[:, 1], lane_color, linewidth=1) # plot centerline
        
        if False: # plotting traffic light colored lanes
            if lane.shape[-1] == 17 and True in list(np.unique(lane[...,13])!=0):
                if len(list(np.unique(lane[...,13])!=0))==1:
                    tl_status = int(np.unique(lane[...,13])[(np.unique(lane[...,13])!=0)][0])
                    tl_color = tl_colors[tl_status]
                    tl_lane = lane[lane[...,13]==tl_status]
                    plt.plot(tl_lane[:, 0], tl_lane[:, 1], tl_color, linewidth=1) # plot centerline
                else:
                    print('')

    
    # if lanes.shape[-1] == 17 and len(np.unique(lanes[...,13]))>1: # plot traffic light controlled lanes
    #     for i in range(lanes.shape[0]):
    #         lane = lanes[i][np.linalg.norm(lanes[i][:, :2], axis=-1)>0]
    #         if len(lane)==0:
    #         continue
    ...
    classes_colors = ['brown', 'b', 'c', 'orange', 'magenta', 'purple', 'pink', 'brown', 'gray']
    ...
    if special_lanes is not None:
        for lane_class, lane in special_lanes:
            ...
            plt.plot(lane[:, 0], lane[:, 1], color=classes_colors[lane_class], 
                    linewidth=5, alpha=0.4, label=classes_names[lane_class])
    ...
    # classes_names = ['stationary', 'straight','straight-right', 'straight-left','right','left','right-u-turn','left-u-turn']
    # if special_lanes is not None:
    #     for lane_class, lane in special_lanes.values():
    #         plt.plot(lane[:, 0], lane[:, 1], classes_colors[lane_class], linewidth=2, alpha=0.5, label='classes_names')
        # print('')

    
    if crosswalks is not None:
        if len(crosswalks.shape)==4:
            crosswalks = crosswalks[0]
        for j in range(crosswalks.shape[0]):
            crosswalk = crosswalks[j]
            if crosswalk[0][0] != 0:
                plt.plot(crosswalk[:, 0], crosswalk[:, 1], 'white', linewidth=1) # plot crosswalk
    
    
    if route_lanes is not None:
        if len(route_lanes.shape)==4:
    ...
    if right_boundaries is not None:
        right_boundaries = right_boundaries[...,:num_lanes,:,:]
        if len(right_boundaries.shape)==4:
            right_boundaries = right_boundaries[0]
        for i in range(right_boundaries.shape[0]):
            boundary = right_boundaries[i]
            if boundary[0][0] != 0:
                plt.plot(boundary[:, 0], boundary[:, 1], 'k', linewidth=1) # plot route_lanes

    if left_boundaries is not None:
        left_boundaries = left_boundaries[...,:num_lanes,:,:]
        if len(left_boundaries.shape)==4:
            left_boundaries = left_boundaries[0]
        for i in range(left_boundaries.shape[0]):
            boundary = left_boundaries[i]
            if boundary[0][0] != 0:
                plt.plot(boundary[:, 0], boundary[:, 1], 'k', linewidth=1) # plot route_lanes
...
def draw_trajectory(ego_trajectory, agent_trajectories):
    # plot ego 
    plt.plot(ego_trajectory[:, 0], ego_trajectory[:, 1], 'b', linewidth=0.5, zorder=3, alpha=1)

    sliced_trajectory = ego_trajectory[::5]
    # Check if the last point is already included, if not, append it
    if ego_trajectory[-1] not in sliced_trajectory:
        sliced_trajectory = np.append(sliced_trajectory, ego_trajectory[-1:], axis=0)
    # Plot circles at the sampled points
    plt.scatter(sliced_trajectory[:, 0], sliced_trajectory[:, 1], s=2, c='r', zorder=5, label='1Hz Sampling')

    # plot others
    for i in range(agent_trajectories.shape[0]):
        if agent_trajectories[i, -1, 0] != 0:
            trajectory = agent_trajectories[i]
            plt.plot(trajectory[:, 0], trajectory[:, 1], 'm', linewidth=3, zorder=3, alpha=0.5)

...
class DataProcess(object):
    ...
    def gen_instruct_caption_02(self, history, future, navigation_extractor, vizualize=True, normalization_param=None):
        ...
        for i in range(len(stop_signs_points)):
            ...
            map_features[f"Stop sign {i+1} location"] = f"({stop_signs_point_local_map[0]:.2f}, {stop_signs_point_local_map[1]:.2f})"
        ...
        for i in range(len(signs_points)):
            ...
            map_features[f"Traffic light {i+1} location"] = f"({traffic_point_local_map[0]:.2f}, {traffic_point_local_map[1]:.2f})"
            map_features[f"Traffic light {i+1} state"] = states_name[signs_states[i]]
    ...
    def gen_instruct_caption_01(self, history, future, navigation_extractor, vizualize=True, normalization_param=None):
    ...
    def map_process(self, traj):
        ...
        if len(traffic_light_lanes)>0:
            traffic_light_distance = [np.linalg.norm(traj[-1, :2] - traffic_light_lanes[k][1:]) for k in traffic_light_lanes.keys()]
            traffic_light_sort_idx = np.argsort(traffic_light_distance)
            sorted_traffic_lights = {k:traffic_light_lanes[k] for k in list(np.array(list(traffic_light_lanes.keys()))[traffic_light_sort_idx])}
            sorted_traffic_lights = np.vstack([[traffic_light_x, traffic_light_y, traffic_light_type] for traffic_light_type, traffic_light_x, traffic_light_y in sorted_traffic_lights.values()])
            ...
            vectorized_traffic_lights[vectorized_traffic_lights[...,-1]==0] = [0,0,0]
            # vectorized_traffic_lights[np.linalg.norm(traj[-1, :2] - vectorized_traffic_lights[...,:2], axis=-1)>311] = [0,0,0]
        # vectorized_stop_signs = np.zeros(shape=(50, 3))
        ...
        for data_file in self.data_files:
            ...
            for data in dataset:
                ...
                for pairs in self.sdc_ids_list:
                    ...
                    max_num_points = 500
                    # map_lanes = np.zeros(shape=(2, self.drivable_lanes, max_num_points, 17), dtype=np.float32) # DONE: make 6 variable
                    ...
                    # continue
                    # t_ = time.time()
                    # map_lanes[0], map_crosswalks[0], map_extra_boundaries_L[0], map_extra_boundaries_R[0], additional_boundaries_[0] = self.map_process(ego[0])
                    map_lanes, map_crosswalks, additional_boundaries_, traffic_lights, stop_signs, speed_bumps = self.map_process(ego[0])
                    # map_extra_boundaries_L[1], map_extra_boundaries_R[1], additional_boundaries_[1] = map_extra_boundaries_L[0], map_extra_boundaries_R[0], additional_boundaries_[0]
                    # map_lanes[1], map_crosswalks[1], _, _ = self.map_process(ego[1])
                    # print(f"map process: {time.time()-t_}")
                    if test:
                    ...
                    # print(f"Interpolate missing data: {time.time()-t_}")
                    if not_valid_future:
                        continue

                    # t_ = time.time()
                    agent_json = self.gen_instruct_caption_01(history=np.vstack((deepcopy(ego),deepcopy(neighbors))),future=deepcopy(ground_truth), navigation_extractor=navigation_extractor, vizualize=not args.not_debug)
                    ...
                    # for agent_i in range(2):
                        # for boundaries in [map_extra_boundaries_L[agent_i], map_extra_boundaries_R[agent_i]]:
                        #     for i_boundary in range(10):
                        #         boundaries[:,:,i_boundary*3:(i_boundary+1)*3] = self.normalize_map_points(boundaries[:,:,i_boundary*3:(i_boundary+1)*3], normalization_param[0], normalization_param[1])
                        # additional_boundaries_[agent_i,:,:,:3] = self.normalize_map_points(additional_boundaries_[agent_i,:,:,:3], normalization_param[0], normalization_param[1])
                    ...
                    if not debug:
                        if test:
                            ...
                            np.savez(filename, ego=np.array(ego), neighbors=np.array(neighbors), map_lanes=np.array(map_lanes)[:n_lanes, :t_samples:t_subsampling, :], 
                            map_crosswalks=np.array(map_crosswalks)[:, :100:t_subsampling],object_type=np.array(object_type),region_6=np.array(region_dict[6]),
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--save_path', type=str, help='path to save processed data', default = '/ibex/project/c2253/felembaa/waymo/gameformer/training_fullmap')
    ...
    parser.add_argument('--save_path', type=str, help='path to save processed data', default = '/ibex/project/c2253/felembaa/waymo/gameformer/validation_fullmap_small')

...
def remove_uturn_after_opposite_turn(entries):
    """
    Removes entries that appear after after right turn if a left U-turn follows,
    or after a left turn if a right U-turn follows.
    
    Parameters:
    - entries (list of tuples): A list where each tuple contains a string (the instruction) and a number (the count).
    
    Returns:
    - list of tuples: A filtered list removing entries after a right turn if a left U-turn follows,
      and after a left turn if a right U-turn follows.
    """
    filtered_entries = []
    ...
    for entry in entries:
        ...
        if "turn right" in instruction and turn_direction=="none":
            ...
            # Do not include anything including this and after it
            break
    ...
    for i in range(len(all_discrete_paths)):
        ...
        for j in range(len(all_discrete_paths[i])):
            ...
            if j>0:
                ...
                # Compute pairwise distances between all points in polyline_to_append and previous_polyline
                distances = np.linalg.norm(polyline_to_append[:, :2, np.newaxis] - previous_polyline[:, :2].T, axis=1)
                # Find the index in polyline_to_append that has the minimum distance to any point in previous_polyline
                closest_index = np.argmin(np.min(distances, axis=1))
                # Slice polyline_to_append starting from closest_index till the end
...
class DataProcess(object):
    ...
    def get_direction_plausibility_03(self, ego_lane, drivable_lanes, ego, map_lanes=None, map_crosswalks=None, ground_truth=None, additional_boundaries_=None, traffic_lights=None, stop_signs=None, agent_json_=None):
        ...
        for i in range(len(possible_map_lanes)):
            if np.any(possible_map_lanes[i][:,0]!=np.inf):
                # Extract valid target lane points
                target_lane = possible_map_lanes[i][possible_map_lanes[i][:, 0] != np.inf]
                if False:
                    # Compute distance difference between ego vehicle and all lane points
                    dist_diff = np.linalg.norm(target_lane[:, :2] - starting_point[:2], axis=-1)
                    # Compute angle difference between ego heading and each lane point heading
                    angle_diff = np.abs(np.degrees(starting_point[-1] - target_lane[:, -1]))
                    # Find indices where distance is within 1 meter
                    # valid_indices = np.where(dist_diff <= 1.0)[0]
                    # Select the closest valid index (if any exist)
                    # if len(valid_indices) > 0:
                        # Among valid points, find the one with the minimum angle difference
                        # best_index = valid_indices[np.argmin(angle_diff[valid_indices])]
                    # else:
                        # Fallback: If no points are within 1 meter, pick the closest point
                        # best_index = np.argmin(dist_diff)
                    # Assign the selected index
                    best_index = np.argmin(angle_diff)
                    target_lane_starting_indx = best_index
                    # if target_lane_starting_indx!=0:
                    #     print()
                    
                    target_lane = target_lane[target_lane_starting_indx:]
                ...
                if len(drivable_target_lane)>4:
                    ...
                    for two_points_lane in [two_points_lane_shortest_path, two_points_lane_quarterway, two_points_lane_halfway, two_points_lane_maximum_distance]:    
                        ...
                        # if 'turn right' in direction_str:
                        #     print()
                    ...
                    # for two_points_lane in [two_points_lane_shortest_path, two_points_lane_quarterway, two_points_lane_halfway, two_points_lane_maximum_distance]:    
                        ...
                        if direction_cls!=-1:
                            ...
                            # starting_point_idx = np.linalg.norm((drivable_target_lane[:,:3] - starting_point)[:,:2], axis=-1).argmin()
                            # # Compute the angle difference between ego heading and lane direction
                            # lane_direction_vector = drivable_target_lane[-1, :2] - drivable_target_lane[0, :2]
                            # ego_vector = np.array([np.cos(starting_point[2]), np.sin(starting_point[2])])  # Ego heading
                            # angle_diff = np.abs(np.degrees(np.arctan2(lane_direction_vector[1], lane_direction_vector[0])) -
                            #     np.degrees(np.arctan2(ego_vector[1], ego_vector[0])))
                            # # Normalize angle difference to [0, 180] range
                            # angle_diff = min(angle_diff, 180 - angle_diff)
                            # # Ensure the lane is aligned within a reasonable heading range (e.g., 30 degrees)
                            # if angle_diff > 30:
                            #     continue  # Skip if lane is facing too differently
                            # # Ensure the vehicle can reach the lane (i.e., it's within a transition distance)
                            # dist_threshold = 100
                            # if np.min(np.linalg.norm(drivable_target_lane[:, :2] - starting_point[:2], axis=-1)) > dist_threshold:
                            #     continue  # Skip if the closest point on the lane is too far away
    ...
    def get_drivable_lanes(self, ego, use_neighbouring_lanes=True):
        ...
        # max_num_neighboring_lanes = 8 ## Lanes that connecect to the ego lane
        ...
        # step 3: Filter all map elements beyond the maximum possible travel range
        # finding max and min possible travel distances
        ...
        # if current_speed<=24: # m/s, replace with 40 m/s for more aggressive stopping
        ...
        # Cropping all lanes based on maximum distance
        lanes_polylines = deepcopy(get_polylines(self.lanes))
        ...
        # max_num_neighboring_lanes = 1 
        if use_neighbouring_lanes: 
            additional_ego_lanes = {} # to current position of the ego vehicle
            ## include all lanes that are within small range to the ego vehicle
            # ego_lane_threshold = 0.5 + get_distance_between_map_object_and_point(ego_coords, sorted_lanes_polylines[ego_lane_id]) ## the distance to the current lane + 0.5m buffer
            ...
            # [get_distance_between_map_object_and_point(ego_coords, sorted_lanes_polylines[key][..., :2]) for key in sorted_lanes_polylines] ## to check the distances
            ...
            ## Include all lanes that are within distance to the ego detected lanes
            ...
            for lane_id in sorted_lanes_polylines:
                ...
                if lane_id == ego_lane_id:
                    continue
                if len(filtered_lanes)>0:
                    ## Include all lanes that are within distance to all ego detected lanes
                    ...
                    ## Include all lanes that are within distance to the ego detected lane
                    object_i_xyh = sorted_lanes_polylines[lane_id]
        ...
        if False:
            colors_list = [
                'teal', 'blue', 'green', 'yellow', 'indigo', 'gold', 'pink', 'brown', 
                'cyan', 'magenta', 'lime', 'teal', 'indigo', 'violet', 'gold', 
                'red', 'blue', 'green', 'yellow', 'purple', 'orange', 'pink', 'brown', 
                'cyan', 'magenta', 'lime', 'teal', 'indigo', 'violet', 'gold', 
                'red', 'blue', 'green', 'yellow', 'purple', 'orange', 'pink', 'brown', 
            ...
            ax, fig = None, None
            for i, k in enumerate(additional_lanes.keys()):
                lane = lanes_polylines[k]
                lane = map_norm(lane, ego_state[:2], ego_state[2])
                fig, ax = vizualize_background(segment=lane, segment_type=0, fig=fig, categ=0, ax=ax, force_color=colors_list[i])
                fig.savefig('ex.png')
                print()
            ...
            for additional_lane_id in additional_lanes:
                ...
                for xx in [np.vstack(_coords_connected[i]) for i in range(len(_coords_connected))]:
                    if True:
                        ...
                        # Find the minimum distance for each point in polyline_to_append to any point in previous_polyline
                        min_distances = np.min(distances, axis=1)
                        # # Find the first index where the distance exceeds the threshold of 0.5
                        # non_intersecting_index = np.argmax(min_distances > 0.5)
                        # Identify non-intersecting points (where distance > 0.5)
                        non_intersecting_mask = min_distances > 0.5
                        
                        # Find the last index where the lane is non-intersecting
                        # If no transitions exist, check if everything is False
                        last_non_intersecting_idx = 0
                        if np.any(non_intersecting_mask):
                            # Find transitions from True → False (start of new False sequences)
                            transitions = np.where((non_intersecting_mask[:-1] == False) & (non_intersecting_mask[1:] == True))[0] + 1
                            if len(transitions) > 0:
                                # Get the last transition (start of the last sequence of Falses)
        ...
        if False:
            ...
            ax, fig = None, None
            for i, lane in enumerate(map_lanes):
                fig, ax = vizualize_background(segment=lane[lane[:,0]!=0], segment_type=0, fig=fig, categ=0, ax=ax, force_color=colors_list[i])
                fig.savefig('ex.png')
                print()
        ...
        ## v1: zeroing > max_distance ## problem, does not spatial-causally slice the lanes, so if the vehicle can take turn and come back from behind that part will be still included
        ...
        # ## v3: Zeroing out points where distance is greater than max_distance
        # for i, lane in enumerate(map_lanes):
        #     distances = np.linalg.norm(lane[:, :2], axis=-1)
        #     exceed_indices = np.where(distances > max_distance)[0]
            
        #     if len(exceed_indices) > 0:
        #         map_lanes[i, exceed_indices] = 0  # Zero out only the exceeding points

                
       
        
        if True:
            ### Code to spatial-causally truncate connected lanes, if an exit lane is connected to a source lane, the exit lane should only have spatial points after the intersection
            ### The code sweep using smaller thresholds and increase to find connected lanes if not found
            ## The lanes within 5 meters to the ego
            for range_threshold in [0.5, 1, 5, 10, 20, 100]:
            # for range_threshold in [1, 2]:
                    ...
                    if i!=j and direct_map_lanes_bool[j] and not direct_map_lanes_bool[i]:
                        ...
                        if len(to_truncate_lane[to_truncate_lane[:,0]!=0])>1 and len(reference_lane[reference_lane[:,0]!=0])>1:
                            ...
                            for range_threshold in [0.01, 0.1, 0.5]:
                                # Find the first occurrence where distance < range_threshold (along reference_lane axis)
                                mask = np.any(distances < range_threshold, axis=0)
                                # Get the first index where mask is True (in filtered_to_truncate_lane)
                                first_index = np.argmax(mask) if np.any(mask) else 0
                                if first_index>0:
                                    break
                            # Get the indices of nonzero rows
                            nonzero_indices = np.nonzero(lane_i[:, 0])[0]
                            # Apply only if we have valid indices and first_index is positive
        ...
        if False:
            ...
            ax, fig = None, None
            for i, lane in enumerate(map_lanes):
                # if colors_list[i] == 'cyan' or colors_list[i] == 'teal' and i in [0,8]:
                fig, ax = vizualize_background(segment=lane[lane[:,0]!=0], segment_type=0, fig=fig, categ=0, ax=ax, force_color=colors_list[i])
            fig.savefig('ex.png')
        ...
        if True:
            ...
            if False:
                if direction_lanes is not None:
                    ...
                    special_lanes = unique_tuples
                    # classes_colors = ['g', 'b', 'c', 'orange', 'yellow', 'purple', 'pink', 'brown']
                    classes_colors = ['brown', 'b', 'pink', 'pink', 'green', 'c', 'yellow', 'brown', 'gray']
                    classes_names = ['stationary', 'straight', 'straight-right', 'straight-left', 'right', 'left', 'right-u-turn', 'left-u-turn']
                    for lane_class, lane in special_lanes:
                        # Use the correct color and class name for the label
                        plt.plot(lane[:, 0], lane[:, 1], color=classes_colors[lane_class], 
                                linewidth=2, alpha=0.7, label=classes_names[lane_class])
    ...
    def process_data(self, viz=True,test=False):
        ...
        pos1_meta_samples_names = [k.split('_interest')[0] for k in pos1_meta.keys()]
        neg1_meta_samples_names = [k.split('_interest')[0] for k in neg1_meta.keys()]
        ...
        for iii, data_file in enumerate(self.data_files):
            ...
            for data in dataset:
                ...
                #     continue
                # if 'f97ffc9772da2734' not in scenario_id:
                #     continue
                ...
                for pairs_idx, pairs in enumerate(self.sdc_ids_list):
                    ...
                    if valid_instruct:
                        ...
                        try:
                            drivable_lanes_new_2, ego_lane_2, max_d_2, min_d_2, vehicle_can_stop_2 = self.get_drivable_lanes(ego[1])
                            valid_instruct_2 = True
                        except:
                            drivable_lanes_new_2, ego_lane_2, max_d_2, min_d_2, vehicle_can_stop_2 = None, None, None, None, None
                            valid_instruct_2 = False
                    ...
                    if valid_instruct:
                        ...
                        if agent_json['Agent-1']['movement 0.1to8']=='INVALID':
                            valid_instruct = False
                        if agent_json['Agent-2']['movement 0.1to8']=='INVALID':
                            valid_instruct_2 = False
                    ...
                    if valid_instruct and args.plausbility:
                        ...
                        if drivable_lanes_new_2 is not None:
                            plausibility_2, directions_lanes_2, _ = self.get_direction_plausibility_03(deepcopy(ego_lane_2), deepcopy(drivable_lanes_new_2), deepcopy(np.stack([ego[1], ego[0]], axis=0)))
                        else:
                            plausibility_2, directions_lanes_2 = None, None
                            # agent_json__2 = {}
                            # agent_json__2['Agent-1'] = agent_json_['Agent-2']
                            # agent_json__2['Agent-2'] = agent_json_['Agent-1']
                            # plausibility_2, directions_lanes_2, drivable_lanes_2 = self.get_direction_plausibility_02(deepcopy(np.stack([ego[1], ego[0]], axis=0)), deepcopy(map_lanes), deepcopy(map_crosswalks), deepcopy(np.stack([ground_truth[1], ground_truth[0]], axis=0)), deepcopy(additional_boundaries_), deepcopy(traffic_lights), deepcopy(stop_signs), deepcopy(agent_json__2))
                        ...
                        if plausibility_2 is not None:
                            possible_directions_2 = plausibility_2
                            possible_directions_cls_2 = [np.where(possible_direction_i == direction_classes)[0][0] for possible_direction_i in possible_directions_2]
                            not_possible_directions_2 = [direction_class_i for direction_class_i in direction_classes if direction_class_i not in plausibility_2]
                            not_possible_directions_cls_2 = [np.where(direction_class_i == direction_classes)[0][0] for direction_class_i in direction_classes if direction_class_i not in plausibility_2]
                        ...
                        if plausibility_2 is not None:
                            # do the same form the _2 entries
                            possible_directions_mask_2 = [agent_json['Agent-2']['direction 0.1to8'] != dir_i for dir_i in possible_directions_2]
                            possible_directions_2 = [possible_directions_2[i] for i, mask in enumerate(possible_directions_mask_2) if mask]
                            possible_directions_cls_2 = [possible_directions_cls_2[i] for i, mask in enumerate(possible_directions_mask_2) if mask]
                            not_possible_directions_mask_2 = [(agent_json['Agent-2']['direction 0.1to8'] != dir_i) or not (dir_i in possible_directions_2) for dir_i in not_possible_directions_2]
                            not_possible_directions_2 = [not_possible_directions_2[i] for i, mask in enumerate(not_possible_directions_mask_2) if mask]
                            not_possible_directions_cls_2 = [not_possible_directions_cls_2[i] for i, mask in enumerate(not_possible_directions_mask_2) if mask]
                        ...
                        if plausibility_2 is not None:
                            if agent_json['Agent-2']['direction 0.1to8']!='stationary':
                                if vehicle_can_stop_2:
                                    if 'stationary' not in possible_directions_2:
                                        possible_directions_2.append('stationary')
                                        possible_directions_cls_2.append(0)
                                    if 'stationary' in not_possible_directions_2:
                                        not_possible_directions_mask_2 = [dir_i != 'stationary' for dir_i in not_possible_directions_2]
                                        not_possible_directions_2 = [not_possible_directions_2[i] for i, mask in enumerate(not_possible_directions_mask_2) if mask]
                                        not_possible_directions_cls_2 = [not_possible_directions_cls_2[i] for i, mask in enumerate(not_possible_directions_mask_2) if mask]
                                else:
                                    if 'stationary' not in not_possible_directions_2:
                                        not_possible_directions_2.append('stationary')
                                        not_possible_directions_cls_2.append(0)
                                    if 'stationary' in possible_directions_2:
                                        possible_directions_mask_2 = [dir_i != 'stationary' for dir_i in possible_directions_2]
                                        possible_directions_2 = [possible_directions_2[i] for i, mask in enumerate(possible_directions_mask_2) if mask]
                                        possible_directions_cls_2 = [possible_directions_cls_2[i] for i, mask in enumerate(possible_directions_mask_2) if mask]
                        ...
                        if templateLLM_filename.split('/')[-1].split('_interest')[0] not in gt1_meta_samples_names:
                            if templateLLM_filename.split('/')[-1].split('_interest')[0] not in pos1_meta_samples_names:
                                if templateLLM_filename.split('/')[-1].split('_interest')[0] not in neg1_meta_samples_names:
                                    # print('xx')
                                    continue
                        ...
                        if valid_instruct:
                            ...
                            try:
                                llm_json = generate_template_json_2agent(agent_json, map_json, possible_directions, not_possible_directions, direction_classes)
                            except:
                                llm_json = generate_template_json(agent_json, map_json, possible_directions, not_possible_directions, direction_classes)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--single_agent', help='data preperation for single agent only', default=False)

...
class DataProcess(object):
    def __init__(
        ...
        self.eval_mode_dir = eval_mode_dir
    ...
    def process_data(self, viz=True,test=False):
        ...
        with open(self.eval_mode_dir, "r") as f:
            valid_json_data = json.load(f)
        valid_scenario_ids = [k.split('_')[0] for k in valid_json_data.keys()]
        valid_scenario_ids_agent1 = ['_'.join(k.split('_')[:2]) for k in valid_json_data.keys()]
        # valid_scenario_ids_agent1_mapping = {'_'.join(k.split('_')[:2]):k for k in valid_json_data.keys()}
        
        # valid_agent_1 = [k.split('_')[1] for k in valid_json_data.keys()]
        # valid_agent_1 = {k.split('_')[0]: k.split('_')[1] for k in valid_json_data.keys()} # given a scenario id
        ...
        for iii, data_file in enumerate(self.data_files):
            ...
            for data in dataset:
                ...
                for pairs_idx, pairs in enumerate(self.sdc_ids_list):
                    ...
                    valid_scenario_ids_agent1
                    if f"{scenario_id}_{sdc_ids[0]}" not in valid_scenario_ids_agent1:
                        continue
                    else:
                        # [f"{scenario_id}_{sdc_ids[0]}" == k for k in valid_scenario_ids_agent1]
                        # if sum([f"{scenario_id}_{sdc_ids[0]}" == k for k in valid_scenario_ids_agent1])==1:
                        #     templatellm_filename_map[mtr_id] =  [k for k in valid_json_data]
                        for k in [k for k in valid_json_data if '_'.join(k.split('_')[:2])==f"{scenario_id}_{sdc_ids[0]}"]:
                            templatellm_filename_map[k] = mtr_id

                    if args.small_data:
                        break

                    if len(templatellm_filename_map) == len(valid_scenario_ids):
                        break
                    else:
                        print(f"{len(templatellm_filename_map)} / {len(valid_scenario_ids)}")
        ...
        # Save JSON files
        if 'gt1' in self.eval_mode_dir:
            eval_mode = 'gt1'
        elif 'neg1' in self.eval_mode_dir:
            eval_mode = 'neg1'
        elif 'pos1' in self.eval_mode_dir:
            eval_mode = 'pos1'

        
        with open(os.path.join(self.templatellm_filename_map_dir, f"{eval_mode}.json"), 'w') as file:
            json.dump(templatellm_filename_map, file, indent=4)

        print(">>>")
        print(">>>")
        print(">>>")
        to_save_dir = os.path.join(self.templatellm_filename_map_dir, f"{eval_mode}.json")
        print(f"SAVED TO: {to_save_dir}")


def parallel_process(root_dir):
    print(root_dir)
    processor = DataProcess(root_dir=[root_dir], point_dir=point_path, save_dir=save_path, eval_mode_dir=eval_mode_dir) 
...
if __name__ == "__main__":
    ...
    eval_mode_dir = args.eval_mode_dir

    if args.use_multiprocessing:
        with Pool(processes=args.processes) as p:
            p.map(parallel_process, data_files)
    else:
        processor = DataProcess(root_dir=data_files, point_dir=point_path, save_dir=save_path, eval_mode_dir=eval_mode_dir)
        processor.process_data(viz=debug,test=test)

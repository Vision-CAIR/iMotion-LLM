...
def training_epoch(train_data, model, optimizer, epoch, act=False, two_agent_act=False):
    ...
    for i in tqdm(range(len(train_data))):
        ...
        # check if 'act' is all -1, then skip this batch (it is of shape (batch_size, 2)), just check if there is at least one number that is not -1 then we are good to go, despite two agent or not
        if act and ((inputs['act']!=-1).sum().item()==0):
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_set', type=str, help='path to train data', default='/ibex/project/c2278/felembaa/datasets/waymo/gameformer/sep14_2025_2a/training')

...
def main():
    ...
    valid_dataset = DrivingData(args.valid_set+'/*', act=True, full_map=args.full_map, contrastive=args.contrastive, positive_notgt=args.positive)
    ...
    for epoch in range(epochs):
        ...
        if not args.eval_only:
            ...
            with torch.no_grad():
                results_dict = validation_epoch_pkl(valid_data, model, epoch, act= args.act or args.act_dec, two_agent_act=False, save_dir=log_path+'results/', num_classes=8, args=args)
                # results_dict = validation_epoch(valid_data, model, epoch, act= args.act or args.act_dec, two_agent_act=False, save_dir=log_path+'results/', num_classes=8, args=args)
...
if __name__ == "__main__":
    ...
    # model
    parser.add_argument("--level", type=int, help='decoder reasoning levels (K)', default=1) # default: 6
    ...
    parser.add_argument("--encoder_layers", type=int, help='encoder layers', default=1)
    ...
    parser.add_argument('--train_set', type=str, help='path to train data', default='/ibex/project/c2278/felembaa/datasets/waymo/gameformer/validation_3jul')
    ...
    parser.add_argument("--two_agents", action="store_true", help='')

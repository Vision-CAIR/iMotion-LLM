...
def main():
    ...
    if not args.eval_only:
        train_dataset = DrivingData(args.train_set+'/*', act=True, full_map=args.full_map, ego_act_only=args.ego_act_only)
        print(f'found {len(train_dataset)} training data')
        training_size = len(train_dataset)
    valid_dataset = DrivingData(args.valid_set+'/*', act=True, full_map=args.full_map, contrastive=args.contrastive, positive_notgt=args.positive, new_eval=args.new_eval, new_eval_mode=args.new_eval_mode, ego_act_only=args.ego_act_only)
    ...
    for epoch in range(epochs):
        ...
        if not args.eval_only:
            ...
            with torch.no_grad():
                results_dict = validation_epoch_pkl(valid_data, model, epoch, act= args.act or args.act_dec, two_agent_act=False, save_dir=log_path+args.new_eval_mode+'results/', num_classes=8, args=args)
                # results_dict = validation_epoch(valid_data, model, epoch, act= args.act or args.act_dec, two_agent_act=False, save_dir=log_path+'results/', num_classes=8, args=args)
...
if __name__ == "__main__":
    ...
    # data & loggings
    parser.add_argument("--workers", type=int, default=0, help="number of workers used for dataloader")
    # model
    parser.add_argument("--level", type=int, help='decoder reasoning levels (K)', default=6) # default: 6
    ...
    parser.add_argument('--lr_steps', type=str, help='', default='10,13,16,19')
    ...
    parser.add_argument('--train_set', type=str, help='path to train data', default='/ibex/project/c2278/felembaa/datasets/waymo/gameformer/training_small_1jul')
    ...
    parser.add_argument('--valid_set', type=str, help='path to train data', default='/ibex/project/c2278/felembaa/datasets/waymo/gameformer/validation_3jul')
    ...
    parser.add_argument('--save_path', type=str, help='name to load ckpts from log path (e.g. epochs_0.pth)', default='')
    ...
    # parser.add_argument("--new_eval_mode", default='neg1')
    parser.add_argument("--new_eval_mode", default='pos1')
    # parser.add_argument('--load_dir', type=str, help='name to load ckpts from log path (e.g. epochs_0.pth)', default='/ibex/project/c2278/felembaa/models/gameformer/cgf_7jul_fulldata/epochs_20.pth')
    # parser.add_argument('--load_dir', type=str, help='name to load ckpts from log path (e.g. epochs_0.pth)', default='/ibex/project/c2278/felembaa/models/gameformer/cgf_11jul_smalldata_2agent/epochs_29.pth')
    parser.add_argument('--load_dir', type=str, help='name to load ckpts from log path (e.g. epochs_0.pth)', default='/ibex/project/c2278/felembaa/models/gameformer/cgf_2jul_smalldata/epochs_29.pth')

...
import time
from gameformer.model.mlp_classifier import GameFormer_
...
class WeightedDistributedSampler(Sampler):
    ...
    def __iter__(self):
        ...
        generator.manual_seed(self.epoch)  # To ensure reproducibility across epochs
        indices = torch.multinomial(
            self.weights, self.total_size, self.replacement, generator=generator
        ).tolist()

        # Partition indices per rank (like DistributedSampler)
...
def initialize_sample_weights_with_known_distribution(data_list):
    """
    ...
    sample_weights = np.full(len(data_list), default_weight, dtype=np.float32)
    ...
    for batch_idx in tqdm(range(num_batches), desc="Computing sample weights"):
        ...
        for idx in range(start_idx, end_idx):
            ...
            if filename in template_cache:
                ...
                act_class = extract_act_for_sample(data_list, idx)
                template_cache[filename] = act_class
                
            if 0 <= act_class < len(act_counts):
                sample_weights[idx] = log_weights[act_class]
                valid_assignments += 1
    ...
    print(f"Weighted sampling initialized with {len(sample_weights)} weights")
    print(f"Valid class assignments: {valid_assignments} ({valid_assignments/len(data_list)*100:.2f}%)")
    print(f"Min weight: {np.min(sample_weights)}, Max weight: {np.max(sample_weights)}")
    print(f"Mean weight: {np.mean(sample_weights)}, Std dev: {np.std(sample_weights)}")
...
def extract_act_for_sample(data_list, idx):
    """
    ...
    """
    data_dir = "/ibex/project/c2278/felembaa/datasets/waymo/gameformer/feb16_2025/training/*"
    try:
        np_filename = data_list[idx]
        root_dir = '/'.join(data_dir.split('/')[:-1])
        filename = np_filename.split('/')[-1].replace('.npz', '')
...
#         (Pdb) batch[7]
        ...
        if 'intent_loss' in outputs:
            alpha = 1.0 
            loss = alpha * outputs['intent_loss']
        
        # back-propagation
        loss.backward(retain_graph=True)
        torch.nn.utils.clip_grad_norm_(model.parameters(), 5)
        # log_dict = {'intent_loss': outputs['intent_loss_scalar']}
        optimizer.step()

        # compute metrics
        current += args.batch_size
        epoch_loss.append(outputs['intent_loss_scalar'])
        metric_logger.update(loss=outputs['intent_loss_scalar'])
...
def main():
    ...
    if args.load_dir != '':
        ...
        model_state_dict = model_ckpts['model_states']
        
        # Load only encoder weights (partial loading)
        encoder_keys = [k for k in model_state_dict.keys() if 'encoder' in k]
        filtered_state_dict = {k: model_state_dict[k] for k in encoder_keys}

        msg = model.load_state_dict(filtered_state_dict, strict=False)  # strict=False allows partial loading
        print("Loaded encoder weights:")
        print(msg)

    # nussair
    for name, param in model.named_parameters():
        if 'encoder' in name and 'mlp' not in name: 
    ...
    if args.weighted_sampling:
        if not args.eval_only:
            ...
            sample_weights = initialize_sample_weights_with_known_distribution(train_dataset.data_list)
            if args.distributed:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weighted_sampling", action="store_true", help='weighted_sampling mode', default=False)
    ...
    parser.add_argument('--train_set', type=str, help='path to train data', default='/ibex/project/c2278/felembaa/datasets/waymo/gameformer/training_28nov')
    ...
    parser.add_argument("--save_model", type=str, help='save model directory, not saved if not provided', default='/home/felembaa/projects/iMotion-LLM-ICLR/gameformer/model/checkpoints')
    ...
    parser.add_argument("--num_act_classes", type=int, default=8)

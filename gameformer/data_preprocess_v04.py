...
def vizualize_background_v01(segment, segment_type, fig, categ, ax):
    ## Guide:
        ...
        for i in range(len(neighbors)):
            if np.any(neighbors[-1,:,0]==0) and sum(neighbors[-1,:,0]!=0)>4:
                ...
                if len(neighbors_interpolated)==0:
                    continue
                neighbors[i, start_non_zero:end_non_zero] = self.interpolate_missing_traj(neighbors[i, start_non_zero:end_non_zero])
        ...
        for data_file in self.data_files:
            ...
            for data in dataset:
                ...
                for pairs in self.sdc_ids_list:
                    ...
                    if not_valid_future:
                        continue
                    agent_json = self.gen_instruct_caption_01(history=np.vstack((deepcopy(ego),deepcopy(neighbors))),future=deepcopy(ground_truth), navigation_extractor=navigation_extractor)
                    ...
                    ego, neighbors, map_lanes, map_crosswalks, ground_truth, region_dict, normalization_param = self.normalize_data(deepcopy(ego), deepcopy(neighbors), deepcopy(map_lanes), deepcopy(map_crosswalks), deepcopy(ground_truth), viz=False)
                    if len(ego)==1:
                        continue

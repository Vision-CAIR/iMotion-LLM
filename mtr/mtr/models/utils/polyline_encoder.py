...
class PointNetPolylineEncoder(nn.Module):
    ...
    def forward(self, polylines, polylines_mask):
        ...
        if polylines_feature.dtype!=polylines_feature_valid.dtype:
            polylines_feature[polylines_mask] = polylines_feature_valid.to(polylines_feature.dtype)
        else:
        ...
        if feature_buffers.dtype!=polylines_feature_valid.dtype:
            feature_buffers[polylines_mask] = polylines_feature_valid.to(feature_buffers.dtype)
        else:
        ...
        if self.out_mlps is not None:
            ...
            if feature_buffers_valid.dtype!=feature_buffers.dtype:
                feature_buffers[valid_mask] = feature_buffers_valid.to(feature_buffers.dtype)    
            else:

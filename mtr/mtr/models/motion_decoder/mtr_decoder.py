...
class MTRDecoder(nn.Module):
    def __init__(self, in_channels, config):
        ...
        if self.model_cfg.ACT:
            self.act_embedding = nn.Embedding(self.num_classes, self.d_model)
            self.act = True
        else:
            self.act = False
    ...
    def apply_dense_future_prediction(self, obj_feature, obj_mask, obj_pos):
        ...
        if ret_obj_feature.dtype!=obj_feature_valid.dtype:
            obj_feature_valid = obj_feature_valid.to(ret_obj_feature.dtype)
    ...
    def get_motion_query(self, center_objects_type):
        ...
        if self.use_place_holder:
            ...
            # Ensure intention_query is on the same device as intention_query_mlps
            target_device = next(self.intention_query_mlps.parameters()).device
            if target_device != intention_query.device:
                intention_query = intention_query.to(target_device)
            if target_device != intention_query.device:
                raise RuntimeError(
                    f"self.intention_query_mlps.device: {target_device} --- intention_query: {intention_query.device}"
                )
    ...
    def apply_cross_attention(self, kv_feature, kv_mask, kv_pos, query_content, query_embed, attention_layer,
        ...
        # Get the device of the attention layer
        target_device = next(attention_layer.parameters()).device
        # Move all inputs to the same device
        query_content = query_content.to(target_device)
        query_embed = query_embed.to(target_device)
        searching_query = searching_query.to(target_device)
        kv_feature = kv_feature.to(target_device)
        kv_mask = kv_mask.to(target_device)
        kv_pos_embed = kv_pos_embed.to(target_device)
    ...
    def apply_transformer_decoder(self, center_objects_feature, center_objects_type, obj_feature, obj_mask, obj_pos, map_feature, map_mask, map_pos, act=None):
        intention_query, intention_points = self.get_motion_query(center_objects_type) # (num_modalities, num_center_objects(2xscenes), latent_dim), _
        if intention_query.dtype!=center_objects_feature.dtype:
            intention_query = intention_query.to(center_objects_feature.dtype)
            intention_points = intention_points.to(center_objects_feature.dtype)
        ...
        # TODO: Add act to intention_query
        if self.act:
            if not (len(act.shape)==2 and act.shape[1]==512):
                act_query = torch.zeros_like(query_content)
                valid_act_mask = act!=-1
                act_query[:,valid_act_mask] = self.act_embedding(act[valid_act_mask])[None]
            else:
                act_query = torch.zeros_like(query_content) # 64,16,512
                act_query = act[None]
            intention_query = intention_query + act_query
        ...
        # TODO: Do we need to add Act here? intention points are identifier for object type (vehicle, pedestrian, cyclist). Make sure it doesn't mean way points
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            # Determine the correct device based on map_pos
            target_device = map_pos.device  
            # Move all inputs to the same device BEFORE calling apply_dynamic_map_collection
            map_pos = map_pos.to(target_device)
            map_mask = map_mask.to(target_device)
            pred_waypoints = pred_waypoints.to(target_device)
    ...
    def get_decoder_loss(self, tb_pre_tag='', reduction='mean'):
        # Ensure everything is moved to CUDA explicitly
        target_device = self.forward_ret_dict['pred_dense_trajs'].device
        # target_device = self.forward_ret_dict['center_gt_trajs'].device  # Pick a reference tensor
        center_gt_trajs = self.forward_ret_dict['center_gt_trajs'].to(target_device)
        center_gt_trajs_mask = self.forward_ret_dict['center_gt_trajs_mask'].to(target_device)
        center_gt_final_valid_idx = self.forward_ret_dict['center_gt_final_valid_idx'].long().to(target_device)
        ...
        intention_points = self.forward_ret_dict['intention_points'].to(target_device)  # (num_center_objects, num_query, 2)

        num_center_objects = center_gt_trajs.shape[0]
        center_gt_goals = center_gt_trajs[torch.arange(num_center_objects), center_gt_final_valid_idx, 0:2].to(target_device)  # (num_center_objects, 2)
        ...
        if reduction!='none':
            ...
            total_loss = torch.zeros(center_gt_trajs.shape[0], device=center_gt_trajs.device)
        for layer_idx in range(self.num_decoder_layers):
            if self.use_place_holder:
                raise NotImplementedError
            ...
            if reduction!='none':
                ...
                total_loss += layer_loss
                tb_dict[f'{tb_pre_tag}loss_layer{layer_idx}'] = layer_loss
                tb_dict[f'{tb_pre_tag}loss_layer{layer_idx}_reg_gmm'] = loss_reg_gmm * weight_reg
                tb_dict[f'{tb_pre_tag}loss_layer{layer_idx}_reg_vel'] = loss_reg_vel * weight_vel
                tb_dict[f'{tb_pre_tag}loss_layer{layer_idx}_cls'] = loss_cls * weight_cls
    ...
    def get_dense_future_prediction_loss(self, tb_pre_tag='', tb_dict=None, disp_dict=None, reduction='mean'):
        ...
        tb_dict[f'{tb_pre_tag}loss_dense_prediction'] = loss_reg.item() if reduction!='none' else loss_reg
        return loss_reg, tb_dict, disp_dict

    def get_loss(self, tb_pre_tag='', reduction='mean'):
        loss_decoder, tb_dict, disp_dict = self.get_decoder_loss(tb_pre_tag=tb_pre_tag, reduction=reduction)
        loss_dense_prediction, tb_dict, disp_dict = self.get_dense_future_prediction_loss(tb_pre_tag=tb_pre_tag, tb_dict=tb_dict, disp_dict=disp_dict, reduction=reduction)

        total_loss = loss_decoder + loss_dense_prediction
        tb_dict[f'{tb_pre_tag}loss'] = total_loss.item() if reduction != 'none' else total_loss
        disp_dict[f'{tb_pre_tag}loss'] = total_loss.item() if reduction != 'none' else total_loss
    ...
    def imotion_forward_step1_(self, batch_dict):
        ...
        if center_objects_feature.dtype!=obj_feature.dtype:
        ...
        if obj_feature_valid.dtype!=obj_feature.dtype:
            obj_feature_valid = obj_feature_valid.to(obj_feature.dtype)
        ...
        if map_feature_valid.dtype!=obj_feature.dtype:
            map_feature_valid = map_feature_valid.to(obj_feature.dtype)
        ...
        # decoder layers
        if self.act:
            act=batch_dict['input_dict']['act'].int().to(obj_feature.device)
        else:
            act=None

        ## TODO: Include iMotion-LLM interaction here, (obj_feature: [batch, 128, 512], )
        ## True is non masked, False is masked
        ## center_objects_feature.shape = [batch, 512]
        ## obj_feature.shape = [batch, 128, 512]
        ## map_feature.shape = [batch, 768, 256]
        return batch_dict, {'center_objects_feature':center_objects_feature, 'obj_feature':obj_feature, 'obj_mask':obj_mask, 'obj_pos':obj_pos, 'map_feature':map_feature, 'map_mask':map_mask, 'map_pos':map_pos}
        # return batch_dict, (center_objects_feature, obj_feature, obj_mask, obj_pos, map_feature, map_mask, map_pos)

    def imotion_forward_step2_(self, batch_dict, center_objects_feature, obj_feature, obj_mask, obj_pos, map_feature, map_mask, map_pos, act=None):
        input_dict = batch_dict['input_dict']
        # dense future prediction
        obj_feature, pred_dense_future_trajs = self.apply_dense_future_prediction(
            obj_feature=obj_feature, obj_mask=obj_mask, obj_pos=obj_pos

...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, training=True, logger=None, fliter_gf_based=False):
        ...
        self.get_act_util_ = get_act_util()
        ...
        # For LLM implementation with preobtained LLM prompts using GameFormer preprocessing code
        if fliter_gf_based: 
            self.selected_data_indexed = self.filter_infos_based_on_gf_data()
            self.logger.info(f'Total scenes after gameformer prompt filters: {len(self.selected_data_indexed)}')
    ...
    def __len__(self):
        if self.new_eval_mode is not None:
    ...
    def __getitem__(self, index):
        ...
        # print(f"__getitem__ loading time for index {index}: {end_time - start_time:.4f} seconds")

        return ret_infos

    def get_new_eval_act(self, filename):
        act = self.meta_eval[filename][self.new_eval_mode]
        acts = np.array([act])
        return acts
    ...
    def create_scene_level_data(self, index):
        ...
        if self.new_eval_mode is not None:
            ...
            ret_infos = self.create_scene_level_data_custom(metadata['infos_index'], metadata['obj_index'])
            
            filename_ = self.selected_data_indexed[index]['filename']
            act = self.meta_eval[filename_][self.new_eval_mode]
            act = np.array([act])
            # act = torch.tensor(act)
            ret_infos.update({'act':act})
            ret_infos.update({'filename':np.array([filename_])})
            return ret_infos
            # index_ = self.selected_data_indexed[index]['infos_index']
            # index=index_
        ...
        ret_dict = {
            ...
            'obj_trajs': obj_trajs_data, # center normalized
        ...
        if not self.dataset_cfg.get('WITHOUT_HDMAP', False):
            ...
            if act is not None:
                ...
                act = np.ones(len(ret_dict['center_gt_trajs_src']))*-1
                try:
                    for batch_i in range(len(ret_dict['center_gt_trajs_src'])):
                        gt_trajs = ret_dict['center_gt_trajs_src'][batch_i, :][:, [0,1,6,7,8]].copy()
                        center, angle = gt_trajs[10][:2].copy(), gt_trajs[10][2].copy()
                        gt_trajs = agent_norm(gt_trajs, center, angle, impute=True)
                        direction = self.get_act_util_.get_act(history=gt_trajs[:11,:5], future=gt_trajs[11:,:5], skip_future_start_end_with_zeros=False, not_skip_invalid_history=False)['Agent-1']['direction 0.1to8_cls']
                        act[batch_i] = direction
                except:
                    pass
                ret_dict['act'] = act
    ...
    def create_scene_level_data_custom(self, index, obj_index):
        """
        Args:
            index (index):

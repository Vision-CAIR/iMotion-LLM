...
def decode_map_features_from_proto(map_features):
    ...
    for cur_data in map_features:
        ...
        if cur_data.lane.ByteSize() > 0:
            ...
            driveway = None # needs to be defined to handle driveway data
        else:
            # if cur_data.driveway.ByteSize()>0:
            #     print("(driveway) data is not handeled, it is part of waymo dataset v1.2.0")
            print(cur_data)
            raise ValueError
...
def process_waymo_data_with_scenario_proto(data_file, output_path=None):
    ...
    for cnt, data in enumerate(dataset):
        ...
        track_infos = decode_tracks_from_proto(scenario.tracks) # objects states
...
def get_infos_from_protos(data_path, output_path=None, num_workers=8):
    ...
    if num_workers>1:
        ...
        for i in tqdm(range(len(src_files))):
            data_infos.append(func(src_files[i]))

    all_infos = [item for infos in data_infos for item in infos]
    return all_infos

def get_infos_from_protos_(data_path, output_path=None, num_workers=8):
    if output_path is not None:
        os.makedirs(output_path, exist_ok=True)
    ...
    for file_path in tqdm(src_files, desc="Processing files"):
        info = process_waymo_data_with_scenario_proto(file_path, output_path=output_path)
        data_infos.extend(info)
...
if __name__ == '__main__':
    ...
    source_dir = '/ibex/project/c2278/felembaa/datasets/waymo'
    target_dir = '/ibex/project/c2253/felembaa/mtr_dataset'

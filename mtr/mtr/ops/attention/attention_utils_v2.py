...
class AttentionWeightComputation(Function):
    ...
    def forward(ctx,
        ...
        if query_features.dtype!=output.dtype:
            query_features = query_features.to(output.dtype)
        if key_features.dtype!=output.dtype:
            key_features = key_features.to(output.dtype)
...
class AttentionValueComputation(Function):
    ...
    def forward(ctx,
        ...
        if value_features.dtype!=output.dtype:
            value_features = value_features.to(output.dtype)

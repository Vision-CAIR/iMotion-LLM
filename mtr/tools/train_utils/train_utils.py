...
def train_one_epoch(model, optimizer, train_loader, accumulated_iter, optim_cfg,
                    ...
                    logger=None, logger_iter_interval=50, cur_epoch=None, total_epochs=None, ckpt_save_dir=None, ckpt_save_time_interval=300, act=False, num_classes=8):
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        if act:
            ## The following was moved to the data loader with fixed issues in the implementation matching the GameFormer preprocessing implementation
            ...
            #         gt_trajs = batch['input_dict']['center_gt_trajs_src'][batch_i].detach()[:, [0,1,6,7,8]].numpy().copy()
            #         center, angle = gt_trajs[10][:2].copy(), gt_trajs[10][2].copy()
            #         gt_trajs = agent_norm(gt_trajs, center, angle, impute=True)
            #         direction = get_act_util_.get_act(history=gt_trajs[:11,:5], future=gt_trajs[11:,:5], skip_future_start_end_with_zeros=False, not_skip_invalid_history=False)['Agent-1']['direction 0.1to8_cls']
            ...
            # instructs_ego_dicts = [navigation_extractor.get_navigation_dict(batch['input_dict']['center_gt_trajs'][batch_i,...,:2].detach().clone())['direction 0.1to8_cls'] for batch_i in range(len(batch['input_dict']['center_gt_trajs'][...,:2]))]
            ...
            # instructs_ego_dicts[-1]['direction 0.1to8_cls']
            batch['input_dict']['act'] = batch['input_dict']['act'].int().to(batch['input_dict']['center_gt_trajs'].device)
            batch['input_dict']['act'][~vehicle_mask] = -1
            # continue
            ## We will match name with gameformer using batch['input_dict']['scenario_id'][i] and batch['input_dict']['center_objects_id'][i]
            # mtr_id = f"{using batch['input_dict']['scenario_id'][0]}_{batch['input_dict']['center_objects_id'][0]}"

            # navigation_extractor.get_navigation_dict(torch.tensor(future_normalized_view))
...
def train_model(model, optimizer, train_loader, optim_cfg,
                ...
                logger_iter_interval=50, ckpt_save_time_interval=300, act=False, num_classes=8):

...
import sys
sys.path.append("..")
# sys.path.append("/home/felembaa/projects/GameFormer/")
import argparse
...
from waymo_preprocess.utils.data_utils import *
...
class DataProcess(object):
    ...
    def normalize_data(self, ego, neighbors, map_lanes, map_crosswalks, ground_truth, viz=False, subsample=True):
        if subsample:
            sampling_index_obs = [int(5*i) for i in range(3)]
            sampling_index_pred = [int(5*i) for i in range(16)]
            if True in (ego[:,sampling_index_obs,0]==0.0) or True in (ground_truth[:,sampling_index_pred,0]==0.0):
                return [], -1, -1, -1, -1,-1
        ...
        if subsample:
            ego = ego[:,sampling_index_obs,:]
            ground_truth = ground_truth[:,sampling_index_pred,:]
        if neighbors:
        ...
        if viz:
            for i in range(ego.shape[0]):
                ...
                past = ego[i]
                # past = ego[i][ego[i][:, 0] != 0]
                plt.plot(past[:, 0], past[:, 1], 'g', linewidth=1, zorder=3)
                future = ground_truth[i]
                # future = ground_truth[i][ground_truth[i][:, 0] != 0]
                plt.plot(future[:, 0], future[:, 1], 'r', linewidth=1, zorder=3, alpha=0.3)
    ...
    def interactive_process(self,tracks_list,interesting_ids,tracks,ooi_only=True):
        self.sdc_ids_list = []
    ...
    def process_data(self, viz=True,test=False):
        ...
        bin_edges, continous_grid, euc_mat = None, None, None
        ...
        for data_file in self.data_files:
            ...
            for data in dataset:
                ...
                if test:
                    ...
                    self.interactive_process(tracks_list, interact_list, parsed_data.tracks, ooi_only=True)
                ...
                for pairs in self.sdc_ids_list:
                    ...
                    ## TODO:
                    # DONE - add rel
                    traj_obs = torch.tensor(ego[:,:,:2])
                    traj_pred = torch.tensor(ground_truth[:,:,:2])
                    rel,_ = traj2rel(torch.cat((traj_obs, traj_pred), dim=1))
                    rel_obs = rel[:,:traj_obs.shape[1],:]
                    rel_pred = rel[:,traj_obs.shape[1]:,:]

                    # - add discrete rel
                    grid, rel_1d, bin_edges, continous_grid, euc_mat  = traj_2_grid_1D(rel, args.num_discrete_anchors, min_val=-18.0, max_val=18.0, bin_edges=bin_edges, continous_grid=continous_grid, euc_mat=euc_mat)
                    rel_obs_1d = rel_1d[:,:traj_obs.shape[1]]
                    rel_pred_1d = rel_1d[:,traj_obs.shape[1]:]
                    ...
                    if test:
                        ...
                        np.savez(filename, ego=np.array(ego), map_lanes=np.array(map_lanes), 
                        map_crosswalks=np.array(map_crosswalks),object_type=np.array(object_type),region_6=np.array(region_dict[6]),
                        object_index=np.array(object_index),current_state=np.array(self.current_xyzh[0]),gt_future_states=np.array(ground_truth),
                        rel_obs=np.array(rel_obs), rel_pred=np.array(rel_pred), rel_obs_1d = np.array(rel_obs_1d), rel_pred_1d = np.array(rel_pred_1d),
                        bin_edges = np.array(bin_edges), continous_grid = np.array(continous_grid), euc_mat=np.array(euc_mat))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path', type=str, help='path to save processed data',default='/home/felembaa/datasets/waymo/training_interactive_processed2')
    ...
    # parser.add_argument('--save_path', type=str, help='path to save processed data',default='/home/felembaa/datasets/waymo/validation_interactive_processed2')
    parser.add_argument('--point_path', type=str, help='path to load K-Means Anchors (Currently not included in the pipeline)', default='')
    parser.add_argument('--processes', type=int, help='multiprocessing process num', default=64)
    ...
    parser.add_argument('--num_discrete_anchors', help='Number of discrete anchors for quantization', default=129)

...
class DataProcess(object):
    ...
    def process_data(self, viz=True,test=False):
        ...
        anchors = linspace_anchors([0,18],args.num_discrete_anchors)
        ...
        for data_file in self.data_files:
            ...
            for data in dataset:
                ...
                for pairs in self.sdc_ids_list:
                    ...
                    rel, traj0 = traj2rel(traj)
                    # - add discrete rel
                    disc_rel = get_signed_grid(rel, anchors)

                    disc_traj = rel2traj(disc_rel, traj0.int())

                    # obs_str = get_waymo_parsed(disc_traj, disc_rel, 3)
                    # pred_str = get_waymo_parsed(disc_traj, disc_rel, 19)

                    np.savez(filename, traj=np.array(traj), rel=np.array(rel), 
                             disc_rel=np.array(disc_rel), disc_traj=np.array(disc_traj),
                             anchors=np.array(anchors), object_type=np.array(object_type)
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_discrete_anchors', help='Number of discrete anchors for quantization', default=200)
    parser.add_argument('--ignore_pairs', action="store_true", help='...', default=False)

...
class MiniGPT4(LlamaForCausalLM):
# class MiniGPT4(AutoModelForCausalLM):
    PRETRAINED_MODEL_CONFIG_DICT = {
        "pretrain_vicuna": "configs/models/minigpt4.yaml",
    }

    def custom_from_pretrained(self, pretrained_model_name_or_path, *args, **kwargs):
        self.lm = self.from_pretrained(pretrained_model_name_or_path, *args, **kwargs)

    # @classmethod
    # def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
    #     # Call the parent class's `from_pretrained` to initialize the instance
    #     instance = super().from_pretrained(pretrained_model_name_or_path, *args, **kwargs)

    #     # You can now add any additional attributes or modifications to `instance`
    #     # instance.custom_attribute = "Custom initialization"
    #     return instance

    # def __init__(self, cfg, *args, **kwargs):
    #     # self.model = super().from_pretrained(pretrained_model_name_or_path, *args, **kwargs)
    ...
    #         *args, **kwargs
    #     )
    #     # Load the pretrained model inside the __init__ method
...
{q} [/INST] {a} {gen_tokens} </s>"""
    ...
    def forward(self, input_ids, _training=True, **kwargs):
        ...
        prompt_tokens = self.llama_tokenizer(prompts_to_tokenize, return_tensors="pt", add_special_tokens=False, padding="longest").to(self.model.embed_tokens.weight.device)
        ...
        if True:
            ...
            if self.gameformer_enc:
                start_scene_token = self.llama_tokenizer(self.scene_start_txt, return_tensors="pt", add_special_tokens=False).to(self.model.embed_tokens.weight.device)
                end_scene_token = self.llama_tokenizer(self.scene_end_txt, return_tensors="pt", add_special_tokens=False).to(self.model.embed_tokens.weight.device)
                ...
                if True:
                    ...
                    start_scene_embed = self.model.embed_tokens(start_scene_token.input_ids)
                    end_scene_embed = self.model.embed_tokens(end_scene_token.input_ids)
                    pad_embed = self.model.embed_tokens(torch.tensor([self.config.pad_token_id], device=self.model.embed_tokens.weight.device))
                    # pad_embed = self.model.embed_tokens(torch.tensor([self.config.pad_token_id]))
            ...
            if True:
                ### MiniGPT-5 codes
                targets_ = self.llama_tokenizer(output_prompts_to_tokenize, return_tensors="pt", add_special_tokens=False, padding="longest").to(self.model.embed_tokens.weight.device)
                ...
                empty_targets = (
                    torch.ones([input_atts.shape[0], input_atts.shape[1]],
                            dtype=torch.long).to(self.model.embed_tokens.weight.device).fill_(-100)
                )
                targets = torch.cat([empty_targets, targets_tokens], dim=1)

                targets_embeds = self.model.embed_tokens(targets_tokens)
                # if not self.lora:
        ...
        ########## end of cleaned code
            ...
            # else: # not training
                ...
                # if True:
                    ...
                    stop_words_ids = torch.tensor([[self.llama_tokenizer.eos_token_id]])
                    ...
                    # output_sequences, output_hidden_states = self.llm_generate(input_embeds, attention_mask, num_words, stopping_criteria=None)
                    ...
                    for i in range(output_sequences.shape[0]):
                        ...
                        if len(output_sequences[i])>k:
                            ...
                            matches = (unfolded == torch.tensor(self.gen_token_idx)).all(dim=1)    
                        else:
                            matches = [False]
                        first_token_idx = -1
                        if True in matches:
                            ...
                            generated_ids_, generated_embeddings_, _ = self.continue_generate(
                                    ...
                                    tokens_to_force_generating=torch.tensor(tokens_to_force_generating),
                            )
                            # generated_ids_, generated_embeddings_, _ = self.continue_generate(
                            #         embeddings=all_previous_embeddings.unsqueeze(0),

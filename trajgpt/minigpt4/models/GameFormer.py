...
class Encoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        actors_mask = torch.eq(actors[:, :, -1].sum(-1), 0) # We can augment this through masking some extra agents
...
class Decoder(nn.Module):
    def __init__(self, modalities, future_len, neighbors_to_predict, levels=6):
    ...
    def forward(self, encoder_inputs):
        ...
        last_scores = torch.stack([result[2] for result in results], dim=1) # [batch, N, M]
...
class GameFormer(nn.Module):
    def __init__(self, modalities, neighbors_to_predict, future_len, encoder_layers=6, decoder_levels=6):

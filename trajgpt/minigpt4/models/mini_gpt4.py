...
class MiniGPT4(Blip2Base):
    ...
    def __init__(self, cfg):
        ...
        self.llm_see_scene_tokens = cfg.model_cfg.get("llm_see_scene_tokens", True)
        self.in_adapter_per_feature = cfg.model_cfg.get("in_adapter_per_feature", True)
    ...
    def __init_additional_modules__(
        ...
        if gameformer_enc:
            ...
            if two_agent: 
                prompt_template_act = '<s>[INST] Predict the future multimodal trajectory embeddings of two agents based on the observed scene embeddings and the given instruction. Instruction: Make the ego vehicle '
            ...
            if self.in_adapter_per_feature:
                if in_adapter_mlp:
                    ...
                    self.llm_input_adapter = nn.ModuleList([nn.Linear(256, lm_hidden_size).to(TRAINABLE_PRECISION) for _ in range(self.gf_token_num)])
            else:
                if in_adapter_mlp:
                    if in_mlp_layers == 4:
                        self.llm_input_adapter = nn.Sequential(
                            nn.Linear(256, 512),
                            nn.GELU(),
                            nn.Linear(512, 512),
                            nn.GELU(),
                    ...
                    self.llm_input_adapter = nn.Linear(256, lm_hidden_size).to(TRAINABLE_PRECISION)
            ...
            if self.output_crossattn:
                ...
                if self.out_adapter_per_embed:
                    ...
                    if out_adapter_mlp:
                        if self.out_mlp_layers == 4:
                            self.llm_output_adapter = nn.Sequential(
                                nn.Linear(lm_hidden_size, 256),
                                nn.GELU(),
                                nn.Linear(256, 512),
                                nn.GELU(),
                        ...
                        self.llm_output_adapter = nn.Linear(lm_hidden_size, 256).to(TRAINABLE_PRECISION)
                # if self.out_adapter_per_embed:
            ...
            if not self.auto_txt_formatter:
            ...
            if self.output_crossattn:
                ...
                ## changing t2i_input_embedding.shape = [batch, self.img_token_num, 256] to context['encodings'].shape = [batch, 2, 114, 256]
                    ...
                    if self.gf_token_num==356:
                        ...
                        raise 'Not implemented'
                    # Nussair
                    elif self.img_token_num==35:
                            num_condition_tokens = 1
                            # t2i_input_embedding: LLM Output
                            condition = t2i_input_embedding[:,:num_condition_tokens,:]

                            # torch.Size([16, 1, 34, 256])
                            t2i_input_embedding = t2i_input_embedding[:,num_condition_tokens:].unsqueeze(1)
                            predicted_context_encodings = t2i_input_embedding
                            predicted_fullmap_encodings = None
                        
                    elif  self.img_token_num==115:
                            num_condition_tokens = 1
                            # t2i_input_embedding: LLM Output
                            condition = t2i_input_embedding[:,:num_condition_tokens,:]
                            t2i_input_embedding = t2i_input_embedding[:,num_condition_tokens:].unsqueeze(1)
                        ...
                        if self.neighbors_to_predict==0:
                            t2i_input_embedding = t2i_input_embedding[:,num_condition_tokens:].reshape(t2i_input_embedding.shape[0],1,self.gf_token_num,t2i_input_embedding.shape[-1])
                            predicted_context_encodings = t2i_input_embedding
                            predicted_fullmap_encodings = None
                        else:
                    ...
                    if self.act_kv:
                        ...
                        gameformer_decoder_input = {'encodings': predicted_context_encodings_,'actors': context['actors'], 'masks': context['masks'], 'act': condition}
                        # Nussair
                    elif predicted_context_encodings.shape[2]==34:
                        # predicted_context_encodings: remaining 114 tokens from the LLM output (tokens)
                        # context['encodings']: scene tokens (114 tokens)
                        predicted_context_encodings_ = torch.zeros_like(context['encodings'])

                        predicted_context_encodings_[:,:,34:,:] = context['encodings'][:,:,34:,:]
                        # torch.Size([16, 1, 114, 256])
                        predicted_context_encodings_[:,0,:34,:] = predicted_context_encodings[:,0,:34,:]
                        if predicted_context_encodings_.shape[1] > 1:  
                            predicted_context_encodings_[:,1,:34,:] = predicted_context_encodings[:,1,:34,:]
                        
                        gameformer_decoder_input = {
                            'encodings': predicted_context_encodings_,
                    ...
                    elif predicted_context_encodings.shape[2]==114:
                        # predicted_context_encodings: remaining 114 tokens from the LLM output (tokens)
                        # context['encodings']: scene tokens (114 tokens)
                        predicted_context_encodings_ = torch.zeros_like(context['encodings'])

                        # this statement is actually doing nothing but I keep it for other cases
                        predicted_context_encodings_[:,:,114:,:] = context['encodings'][:,:,114:,:]
                        # torch.Size([16, 1, 114, 256])
                        predicted_context_encodings_[:,0,:114,:] = predicted_context_encodings[:,0,:114,:]
                        if predicted_context_encodings_.shape[1] > 1:  
                            predicted_context_encodings_[:,1,:114,:] = predicted_context_encodings[:,1,:114,:]
    ...
    def _gf_post_processing(self, outputs=None, context=None, input_adapted_context_embs=None, 
                      ego_future=None, neighbor_future=None, level=3, subsample=False, 
                      reduction='mean', is_eval=False, t2i_input_embedding=None):
        """
        Unified post-processing function for both training and evaluation.
        
        Args:
            outputs: Model outputs (required for training, not for eval)
            context: Context information
            input_adapted_context_embs: Adapted context embeddings
            ego_future: Future trajectory for ego vehicle
            neighbor_future: Future trajectory for neighbor vehicles
            level: Level parameter for level_k_loss
            subsample: Subsample parameter for level_k_loss
            reduction: Reduction method for loss calculation (only used in training)
            is_eval: Whether in evaluation mode
            t2i_input_embedding: Pre-processed embeddings (can be provided directly in eval mode)
        
        Returns:
            For training: loss, future, recon_losses
            For evaluation: loss, future, trajectories, scores
        """
        # Get embeddings from outputs in training mode
        if not is_eval:
            # text_loss = outputs["loss"]
            last_hidden_state = outputs['hidden_states'][-1]
            special_token_index = outputs['output_token_index']
            ...
            if len(t2i_input_embedding) == 0:
                return -1  # Early return if no embeddings
            else:
                t2i_input_embedding = torch.cat(t2i_input_embedding, dim=0)
        ...
        # LLM dimension data recon loss (only in training)
        if not is_eval and self.recon_loss_llm:
            if self.output_crossattn:
                raise 'not implemented'
            llm_out_emb = t2i_input_embedding.reshape(t2i_input_embedding.shape[0], 2, 
                                                    int(t2i_input_embedding.shape[1]/2), 
                                                    t2i_input_embedding.shape[-1])
            if llm_out_emb.shape != input_adapted_context_embs.shape:
                raise 'not implemented'
            lmm_recon_loss = F.mse_loss(llm_out_emb, input_adapted_context_embs, reduction='mean')
            recon_losses[0] = lmm_recon_loss
        ...
        if self.output_crossattn:
            ...
            # Legacy branch - keeping for compatibility
            if False:  # TODO: Check if needed
                query_act = t2i_input_embedding[:, 0]
                scene_act = t2i_input_embedding[:, 1]
                t2i_input_embedding_ = t2i_input_embedding[:, 2:]
                t2i_input_embedding_ = t2i_input_embedding_.reshape(t2i_input_embedding_.shape[0], 2, 
                                                                int(t2i_input_embedding_.shape[1]/2), 
                                                                t2i_input_embedding_.shape[-1])
                gameformer_decoder_input = {'encodings': t2i_input_embedding_,
                                        'actors': context['actors'], 
                                        'masks': context['masks'], 
                                        'act': scene_act, 
                                        'query_act': query_act}
                gameformer_decoder_input = self.gameformer_model.encoder_02(gameformer_decoder_input)
                decoded_output = self.gameformer_model.decoder(gameformer_decoder_input)
                loss, future, _ = level_k_loss(decoded_output, ego_future, neighbor_future, level, 
                                            subsample=subsample)
            else:
                # Handle different token configurations
                num_condition_tokens = self.img_token_num - self.gf_token_num
                if num_condition_tokens>0: # number of img tokens is larger the same as number of gf tokens, if we are generating all tokens
                    condition = t2i_input_embedding[:,:num_condition_tokens,:]
                else:
                ...
                # Token handling following the original training code order
                num_condition_tokens = self.img_token_num - self.gf_token_num
                if num_condition_tokens > 0:  # number of img tokens is larger than number of gf tokens
                    condition = t2i_input_embedding[:, :num_condition_tokens, :]
                else:
                    condition = None
                ...
                if self.gf_token_num==356:
                    ...
                    raise 'Not implemented'
                        ...
                        predicted_fullmap_encodings = None
                    # Nussair
                elif self.img_token_num==61:
                        num_condition_tokens = 1
                        # t2i_input_embedding: LLM Output
                        condition = t2i_input_embedding[:,:num_condition_tokens,:]

                        # torch.Size([16, 1, 60, 256])
                        t2i_input_embedding = t2i_input_embedding[:,num_condition_tokens:].unsqueeze(1)
                        predicted_context_encodings = t2i_input_embedding
                        predicted_fullmap_encodings = None
                elif self.img_token_num==21:
                        num_condition_tokens = 1
                        # t2i_input_embedding: LLM Output
                        condition = t2i_input_embedding[:,:num_condition_tokens,:]
                ...
                elif predicted_context_encodings.shape[2]==60:
                    # predicted_context_encodings: remaining 114 tokens from the LLM output (tokens)
                    # context['encodings']: scene tokens (114 tokens)
                    predicted_context_encodings_ = torch.zeros_like(context['encodings'])

                    # take all of them from the scene mapper
                    predicted_context_encodings_ = context['encodings']
                    
                    # slice the 60 tokens from the middle of the LLM output
                    predicted_context_encodings_[:,0,34:94,:] = predicted_context_encodings[:,0,:60,:]
                    if predicted_context_encodings_.shape[1] > 1:  
                        predicted_context_encodings_[:,1,34:94,:] = predicted_context_encodings[:,0,:60,:]
                    gameformer_decoder_input = {
                        'encodings': predicted_context_encodings_,
                        'actors': context['actors'],
                        'masks': context['masks'],
                ...
                elif predicted_context_encodings.shape[2]==20:
                    # predicted_context_encodings: remaining 114 tokens from the LLM output (tokens)
                    # context['encodings']: scene tokens (114 tokens)
                    predicted_context_encodings_ = torch.zeros_like(context['encodings'])

                    # take the first 94 from the scene mapper
                    predicted_context_encodings_[:,:,:94,:] = context['encodings'][:,:,:94,:]
                
                    # take the rest from the LLM output
                    predicted_context_encodings_[:,0,94:,:] = predicted_context_encodings[:,0,:20,:]
                    if predicted_context_encodings_.shape[1] > 1:  
                        predicted_context_encodings_[:,1,94:,:] = predicted_context_encodings[:,0,:20,:]
                ...
                # Calculate loss and trajectories
                if is_eval:
                    # Evaluation mode - use level_k_loss without extra parameters
                    # and return trajectories and scores
                    if ego_future is not None:
                        loss, future, trajectories = level_k_loss(decoded_output, ego_future, neighbor_future, 
                                                                level, subsample=subsample)
                        scores = decoded_output[f'level_{level}_scores']
                        return loss, future, trajectories, scores
                    else:
                        return level_k_loss(decoded_output, ego_future, neighbor_future, 
                                                                level, subsample=subsample)

                else:
                    # Training mode - use additional parameters and return recon_losses
                    # loss, future, trajectories = level_k_loss(decoded_output, ego_future, neighbor_future, 
                    #                                         level, subsample=subsample, reduction=reduction,
                    #                                         two_agent_reduction='none' if self.neighbors_to_predict > 0 else 'yes')
                    loss, future, _ = level_k_loss(decoded_output, ego_future, neighbor_future, level, subsample=subsample, reduction=reduction, two_agent_reduction='none' if self.neighbors_to_predict>0 else 'yes')
                    return loss, future, recon_losses
...
{q} [/INST] {a} {gen_tokens} </s>"""
    ...
    # def forward(self, samples, _training=True):
    def closed_loop_inference(self, input_features):
        ...
        samples = input_ids
        local_batch_size = samples['ego_state'].shape[0]
        direction_act = input_features['act'][0][0]
        if 'act_complex' in input_features:
            samples['instruct'] = input_features['act_complex']
        if direction_act!=-1:
            direction_str_list = ['stay stationary', 'move straight', 'turn right', 'turn left', 'take a left u-turn']
            samples['instruct'] = [f"<s>[INST] Generate trajectories embeddings based on the following given instruction and scene embeddings. Make the ego vehicle {direction_str_list[direction_act]}. "]
        else:
            samples['instruct'] = [f"<s>[INST] Generate trajectories embeddings based on the following scene embeddings. "]
        prompts_act = []
        prompts_act_out = []
        # template_data = []
        for i, instruct in enumerate(samples['instruct']):
            ...
            batch_size=1
            if self.gameformer_enc and self.llm_see_scene_tokens:
            ...
            trajectories, scores =  self._gf_post_processing(is_eval=True, t2i_input_embedding = output_embeddings, context=context, input_adapted_context_embs = context_embs, ego_future=None, neighbor_future=None, level=self.gf_level)
        return {'modalities': trajectories, 'scores': scores, 'text': text_out}
        ...
        if True:
            ...
            if True:
                ...
                if _training:
                    ...
                    gf_loss, gf_future, recon_losses = self._gf_post_processing(outputs_dict, context, context_embs, ego_future=samples['ground_truth'][:,0,:,:], neighbor_future=samples['ground_truth'][:,1,:,:], level=self.gf_level, subsample=False, reduction=reduction)
        ...
        ########## end of cleaned code
            ...
            # else: # not training
                ...
                # if True:
                    ...
                    # text_out, output_embeddings = self.old_generate(input_embeds, input_atts, 100)
                    text_out, output_embeddings = self.old_generate(input_embeds, input_atts, 2)
                    gf_loss, gf_future, trajectories, gf_scores =  self._gf_post_processing(is_eval=True, t2i_input_embedding = output_embeddings, context=context, input_adapted_context_embs = context_embs, ego_future=samples['ground_truth'][:,0,:,:], neighbor_future=samples['ground_truth'][:,1,:,:], level=self.gf_level)
    ...
    def generate(self, embeddings=torch.FloatTensor, max_len: int = 15,
        ...
        with torch.no_grad():  # no tracking history
            ...
            for i in range(max_len):
                ...
                end_critria = [last_found_txt in [self.llama_tokenizer.eos_token, '$$'] for last_found_txt in self.llama_tokenizer.batch_decode(next_token)]
    ...
    # def from_config(self, cfg):
        ...
        self.__init_additional_modules__(
            ...
            in_mlp_layers = cfg.get("in_mlp_layers", 2),
            out_mlp_layers = cfg.get("out_mlp_layers", 2),
        )
        # model = cls(
        #     vit_model=vit_model,
        #     q_former_model=q_former_model,

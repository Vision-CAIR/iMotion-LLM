...
class FeatureQformer(nn.Module):
    def __init__(self, feature_dim=512, query_dim=768, num_queries=114, 
                 lm_hidden_size=4096):
        super().__init__()
        ...
        self.encoder_config.encoder_width = feature_dim
        self.encoder_config.hidden_size = query_dim
        self.encoder_config.num_hidden_layers = 6  # Use fewer layers for efficiency
        
        # Initialize query tokens (learnable)
        ...
        self.feature_proj = nn.Linear(feature_dim, feature_dim) if feature_dim != self.encoder_config.encoder_width else nn.Identity()
        
        # Use a custom cross-attention implementation
        self.cross_attention_layers = nn.ModuleList([
            CrossAttentionLayer(query_dim, feature_dim) 
            for _ in range(self.encoder_config.num_hidden_layers)
        ])
        
        # Projection to LLM dimension
        self.llm_proj = nn.Linear(query_dim, lm_hidden_size)
        
    def forward(self, features, feature_masks=None):
        """
        Forward pass to transform input features into LLM tokens
        """
        ...
        for layer in self.cross_attention_layers:
            hidden_states = layer(hidden_states, features, attention_mask=feature_masks)
        
        # Project to LLM dimension
        llm_features = self.llm_proj(hidden_states)
        
        # Safety check for NaN values
        if torch.isnan(llm_features).any():
            print("WARNING: NaN values detected in FeatureQformer output")
            llm_features = torch.nan_to_num(llm_features, nan=0.0)
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states, attention_mask=None):
        """
        Apply self-attention followed by cross-attention and feed-forward network
        
        Args:
            hidden_states: [batch_size, seq_len, hidden_size] - Query tokens
            encoder_hidden_states: [batch_size, enc_seq_len, enc_hidden_size] - Key/Value from encoder
            attention_mask: [batch_size, enc_seq_len] - Boolean mask (True for valid positions)
        """
        # Convert boolean mask to attention mask
        # For MultiheadAttention, mask values of True indicate positions to be ignored
        if attention_mask is not None:
            cross_attn_mask = ~attention_mask  # Invert mask for PyTorch attention
            # PyTorch attention expects a float mask with -inf for masked positions
            cross_attn_mask = cross_attn_mask.float().masked_fill(
                cross_attn_mask, float('-inf')
            )
        else:
        ...
        hidden_states = self.self_attention_layer_norm(hidden_states + residual)
        ...
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.cross_attention_layer_norm(hidden_states + residual)
        
        # Feed-forward
        residual = hidden_states
        hidden_states = self.ffn(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.ffn_layer_norm(hidden_states + residual)
...
def get_module_device(module):
    for param in module.parameters():
        return param.device
    return torch.device("cpu")  # Default if there are no parameters
...
##############################
##############################
@registry.register_model("gameformer_gpt_mtr")
class IMotionLLMMTRModel(Blip2Base):
    """
    ...
    def __init__(self, cfg):
        super().__init__()
        self.llm_name = cfg.model_cfg.llama_model
    ...
    def __init_additional_modules__(
        ...
        attention_input_adapter = False,
        attention_input_adapter_numQ = 114,
        mtr_cfg_path=None,
        mtr_ckpt_path=None,
        mtr_intention_points_file=None,
    ):
        self.attention_input_adapter = attention_input_adapter
        self.mtr_cfg_path = mtr_cfg_path
        self.mtr_ckpt_path = mtr_ckpt_path
        self.mtr_intention_points_file = mtr_intention_points_file
        self.no_sys_prompt = no_sys_prompt
        self.auto_txt_formatter=auto_txt_formatter
        self.act_kv = act_kv
        ...
        if gameformer_enc:
            ...
            if mtr:
                ...
                self.load_mtr()
                self.use_mtr=True
                self.mtr.motion_decoder.act_embedding = nn.Identity()
            else:
                self.use_mtr=False
        ...
        if auto_txt_formatter:
            ...
            if 'Qwen' in self.llm_name and self.llama_tokenizer.bos_token is None:
        ...
        if gameformer_enc:
            if not mtr:
                ...
                if freeze_gf or eval_only:
                    self.mtr.requires_grad_(False)
                else:
                    if freeze_gf_enc:
                        self.mtr.context_encoder.requires_grad_(False)
                    if freeze_gf_dec:
                        self.mtr.motion_decoder.requires_grad_(False)
                    # self.mtr.requires_grad_(True)
                    # if freeze_gf_enc:
                    #     self.gameformer_model.encoder_01.requires_grad_(False)
                    #     self.gameformer_model.encoder_02.requires_grad_(False)
                    # if freeze_gf_dec:
                    #     self.gameformer_model.decoder.requires_grad_(False)
                print('> loading MTR Done')
            ...
            if not mtr:
                ...
                if attention_input_adapter_numQ:
                    self.llm_input_adapter = nn.Linear(512, 256).to(TRAINABLE_PRECISION)
                    self.llm_input_adapter2 = FeatureQformer(
                        feature_dim=256,
                        query_dim=768,  # BERT hidden size
                        num_queries=114,
                        lm_hidden_size=lm_hidden_size
                    ).to(TRAINABLE_PRECISION)
                    # self.agent_projection = nn.Linear(512, 256).to(TRAINABLE_PRECISION)
                    # self.feature_query_transformer = FeatureQueryTransformer(
                    #     feature_dim=256,
                    #     query_dim=512,  # intermediate size
                    #     num_queries=114,  # number of summary tokens
                    #     num_heads=8,
                    #     num_layers=2,
                    #     lm_hidden_size=lm_hidden_size,
                    # ).to(TRAINABLE_PRECISION)
                elif self.in_adapter_per_feature:
                    if in_adapter_mlp:
                        self.llm_input_adapter =  nn.ModuleList([
                            nn.Sequential(
                            nn.Linear(512, 256),
                            nn.GELU(),
                            nn.Linear(256, lm_hidden_size),
                        ).to(TRAINABLE_PRECISION) for _ in range(33)])
                        self.llm_input_adapter2 =  nn.ModuleList([
                            nn.Sequential(
                            nn.Linear(256, 256),
                            nn.GELU(),
                            nn.Linear(256, lm_hidden_size),
                        ).to(TRAINABLE_PRECISION) for _ in range(81)])
                    else:
                        self.use_some_of_the_scene_features = False
                        if self.use_some_of_the_scene_features:
                            self.llm_input_adapter = nn.ModuleList([nn.Linear(512, lm_hidden_size).to(TRAINABLE_PRECISION) for _ in range(33)]) # could be replaced with a transformer
                            self.llm_input_adapter2 = nn.ModuleList([nn.Linear(256, lm_hidden_size).to(TRAINABLE_PRECISION) for _ in range(81)]) # could be replaced with a transformer
                        else:    
                            self.llm_input_adapter = nn.ModuleList([nn.Linear(512, lm_hidden_size).to(TRAINABLE_PRECISION) for _ in range(71+1)]) # could be replaced with a transformer
                            self.llm_input_adapter2 = nn.ModuleList([nn.Linear(256, lm_hidden_size).to(TRAINABLE_PRECISION) for _ in range(768)]) # could be replaced with a transformer
                        
                else:
                    self.llm_input_adapter = nn.Linear(512, lm_hidden_size).to(TRAINABLE_PRECISION) # could be replaced with a transformer
                    self.llm_input_adapter2 = nn.Linear(256, lm_hidden_size).to(TRAINABLE_PRECISION) # could be replaced with a transformer
                ...
                if self.out_adapter_per_embed:
                    ...
                    if out_adapter_mlp:
                        self.llm_output_adapter = nn.ModuleList([
                            ...
                            nn.GELU(),
                            nn.Linear(256, 512 if mtr else 256),
                        ).to(TRAINABLE_PRECISION) for _ in range(IMG_TOKEN_NUM)])
                    else:   
                        self.llm_output_adapter = nn.ModuleList([nn.Linear(lm_hidden_size, 512 if mtr else 256).to(TRAINABLE_PRECISION) for _ in range(IMG_TOKEN_NUM)])
                else:
                    self.llm_output_adapter = nn.Linear(lm_hidden_size, 512 if mtr else 256).to(TRAINABLE_PRECISION) # could be replaced with a transformer
        ...
        if not mtr:
            ...
            self.mtr.to(self.llm.model.embed_tokens.weight.device)
            self.print_trainable_parameters(self.mtr)
        print('input_mapping:')
        ...
        print('MTR encoder:')
        self.print_trainable_parameters(self.mtr.context_encoder)
        print('MTR decoder:')
        self.print_trainable_parameters(self.mtr.motion_decoder)
        print('')
    ...
    def mtr_post_processing_test(self, t2i_input_embedding, context, input_adapted_context_embs, batch_dict, other_features, subsample=False, reduction='mean'):
        img_token_bs = t2i_input_embedding.shape[0]
        # recon loss
        recon_losses = [0,0]
        ...
        if self.output_crossattn:
            ...
            if self.act_kv:
                ...
                # loss,_,_ = self.mtr.imotion_forward_step2(batch_dict, other_features, condition[:,0], reduction=reduction)
                # return loss,None,None
                output_dict = self.mtr.imotion_forward_step2(batch_dict, other_features, condition[:,0], reduction=reduction)
                return output_dict
            elif predicted_context_encodings.shape[2]==1:
                # Replace the corresponding variables of interest before passing to the later decoding stage of MTR 
                other_features['center_objects_feature'] = predicted_context_encodings[:,0,0]
                if get_module_device(self.mtr) != other_features['center_objects_feature'].device:
                    raise f"self.mtr.device : {get_module_device(self.mtr)} != other_features['center_objects_feature'].device : {other_features['center_objects_feature'].device}"
                output_dict = self.mtr.imotion_forward_step2(batch_dict, other_features, condition[:,0], reduction=reduction)
                return output_dict
                # return loss,None,None
                if False:
    ...
    def mtr_post_processing(self, outputs, context, input_adapted_context_embs, batch_dict, other_features, subsample=False, reduction='mean'):
        # text_loss = outputs["loss"]
        last_hidden_state = outputs['hidden_states'][-1]
        special_token_index = outputs['output_token_index']
        t2i_input_embedding = []
            ...
            if self.output_crossattn:
                ...
                ## changing t2i_input_embedding.shape = [batch, self.img_token_num, 256] to context['encodings'].shape = [batch, 2, 114, 256]
                    ...
                    if self.act_kv:
                        ...
                        # Replace the corresponding variables of interest before passing to the later decoding stage of MTR 
                        other_features['center_objects_feature'] = predicted_context_encodings[:,0,0] # TODO: Check if this is all what we need to replace, or other features need to be replaced in MTR.
                        if get_module_device(self.mtr) != other_features['center_objects_feature'].device:
                            raise f"self.mtr.device : {get_module_device(self.mtr)} != other_features['center_objects_feature'].device : {other_features['center_objects_feature'].device}"
                        loss,_,_ = self.mtr.imotion_forward_step2(batch_dict, other_features, condition[:,0], reduction=reduction)
                        return loss,None,None
...
{q} [/INST] {a} {gen_tokens} </s>"""
    ...
    # def forward(self, samples, _training=True):
    def universal_template(self,q,select):
        if select==1: # meta-llama/Llama-2-7b-hf, meta-llama/Llama-2-13b-hf, mistralai/Mistral-7B-Instruct-v0.1
            return f"<s>[INST] {q} "
        elif select==2: # meta-llama/Meta-Llama-3-instruct
            return f"<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n{q} "
        elif select==3: # lmsys/vicuna-7b-v1.5
            return f"USER: {q} "
        elif select==4: # Qwen/Qwen2-7B-Instruct
            return f"<|im_start|>user\n{q} "

    def mtr_forward(self, input_ids, _training=True):
        # self.mtr(input_ids)
        batch_dict, other_features = self.mtr.imotion_forward_step1(input_ids)
        # other_features: (center_objects_feature, obj_feature, obj_mask, obj_pos, map_feature, map_mask, map_pos)
        # center_objects_feature, obj_feature, obj_mask, obj_pos, map_feature, map_mask, map_pos = other_features
        ...
        center_objects_feature, obj_feature, obj_mask, obj_pos, map_feature, map_mask, map_pos = center_objects_feature.to(embed_device), obj_feature.to(embed_device), obj_mask.to(embed_device), obj_pos.to(embed_device), map_feature.to(embed_device), map_mask.to(embed_device), map_pos.to(embed_device)
        samples = input_ids['input_dict']
        local_batch_size = samples['act'].shape[0]
        if local_batch_size < len(samples['instruct']):
            ...
            raise "Check if other things needs to be scattered as well"

        self.llama_tokenizer.padding_side = "right"
        batch_size = samples['act'].shape[0]
        ...
        for i, instruct in enumerate(samples['instruct']):
            ...
            if self.use_llama2_template:
                ...
                if self.auto_txt_formatter:
                    if 'Llama-2' in self.llm_name or 'Mistral' in self.llm_name:
                        prompts_act.append(self.universal_template(llama2_template_data_q, 1))
                    elif 'Llama-3' in self.llm_name:
                        prompts_act.append(self.universal_template(llama2_template_data_q, 2))
                    elif 'vicuna' in self.llm_name:
                        prompts_act.append(self.universal_template(llama2_template_data_q, 3))
                    elif 'Qwen' in self.llm_name:
                        prompts_act.append(self.universal_template(llama2_template_data_q, 4))
                else:
        ...
        if self.use_llama2_template:
            ...
            if 'Llama-2' in self.llm_name or not self.auto_txt_formatter or 'Mistral' in self.llm_name:
                end_input_token = ' [/INST]' 
            elif 'Llama-3' in self.llm_name:
                end_input_token = f'{self.llama_tokenizer.eos_token}\n<|start_header_id|>assistant<|end_header_id|>\n\n' 
            elif 'vicuna' in self.llm_name:
                end_input_token = '\nAssistant: '
            elif 'Qwen' in self.llm_name:
                end_input_token = f'\n{self.llama_tokenizer.eos_token}\n{self.llama_tokenizer.bos_token}assistant\n'
            end_scene_token = self.llama_tokenizer(self.scene_end_txt+end_input_token, return_tensors="pt", add_special_tokens=False).to(embed_device)
        
        # context_embs = center_objects_feature, obj_feature, map_feature
        # center_objects_feature, obj_feature, obj_mask, obj_pos, map_feature, map_mask, map_pos
        
        if self.attention_input_adapter:
            # Project center objects feature (which is 512-dim) to 256-dim
            center_objects_feature_proj = self.llm_input_adapter(center_objects_feature)
            # First combine all features and their masks
            # Need to project agent features to match map feature dimensions
            all_features = torch.cat([
                center_objects_feature_proj.unsqueeze(1),
                self.llm_input_adapter(obj_feature),  # Project obj_feature (512-dim) to 256-dim
                map_feature  # Already 256-dim
            ], dim=1)
            all_masks = torch.cat([
                torch.ones((center_objects_feature.shape[0], 1), device=obj_mask.device, dtype=torch.bool),
                obj_mask,
                map_mask
            ], dim=1)
            # Apply query transformer to summarize all features into a fixed number of tokens
            context_embs = self.llm_input_adapter2(all_features, feature_masks=all_masks)
            
            # Create attention mask for the transformer output (all tokens are valid)
            context_atts = torch.ones((batch_size, context_embs.shape[1]), dtype=torch.int, device=embed_device)
            combined_masks = torch.zeros((batch_size, context_embs.shape[1]), dtype=torch.bool, device=embed_device)
        elif not self.use_some_of_the_scene_features:
            context_embs_llm_maped = []
            context_embs_llm_maped.append(self.llm_input_adapter[0](center_objects_feature).unsqueeze(1))
            for adapter_i in range(1, len(self.llm_input_adapter)):
                context_embs_llm_maped.append(self.llm_input_adapter[adapter_i](obj_feature[:,adapter_i-1:adapter_i])) # we are only using subset of obj_feature (total: 71)
            for adapter_i in range(0, len(self.llm_input_adapter2)):
                context_embs_llm_maped.append(self.llm_input_adapter2[adapter_i](map_feature[:,adapter_i:adapter_i+1])) # we are only using subset of map_feature (total: 768)
            context_embs = torch.stack(context_embs_llm_maped, dim=1).squeeze(2)
            combined_masks = torch.cat((torch.tensor([[True]]*context_embs.shape[0], device=obj_mask.device), obj_mask, map_mask), axis=1)
            # combined_masks = torch.cat((torch.tensor([True], device=obj_mask.device), obj_mask.any(0), map_mask.any(0)))
            context_atts = combined_masks.int()
        else: # using some of the scene encodings features not all of them
            context_embs_llm_maped = []
            context_embs_llm_maped.append(self.llm_input_adapter[0](center_objects_feature).unsqueeze(1))
            for adapter_i in range(1, len(self.llm_input_adapter)):
                context_embs_llm_maped.append(self.llm_input_adapter[adapter_i](obj_feature[:,:32][:,adapter_i-1:adapter_i])) # we are only using subset of obj_feature (total: 71)
            for adapter_i in range(0, len(self.llm_input_adapter2)):
                context_embs_llm_maped.append(self.llm_input_adapter2[adapter_i](map_feature[:,:81][:,adapter_i:adapter_i+1])) # we are only using subset of map_feature (total: 768)
            context_embs = torch.stack(context_embs_llm_maped, dim=1).squeeze(2)
            combined_masks = torch.cat((torch.tensor([True], device=obj_mask.device), obj_mask[:,:32].any(0), map_mask[:,:81].any(0)))
            context_atts = combined_masks.int()
        ...
        context_embs[~combined_masks] = pad_embed
        # context_embs[:,~combined_masks] = pad_embed
        ...
        input_atts = torch.cat((
            ...
            # expanded_stack(context_atts,batch_size),
            context_atts,
        ...
        if _training:
            ...
            # gf_loss, gf_future, recon_losses = self.mtr_post_processing(outputs_dict, None, context_embs, batch_dict=batch_dict, other_features=other_features, subsample=False, reduction=reduction)
            gf_loss, gf_future, recon_losses = self.mtr_post_processing(outputs_dict, None, context_embs, batch_dict=batch_dict, other_features=other_features, subsample=False, reduction=reduction)
            traj_loss = gf_loss*~samples['contrastive_sample']
            traj_loss = traj_loss.mean(-1)

            total_loss = loss*10 + traj_loss
            outputs_dict = {"loss": total_loss}
        else:
            text_out, output_embeddings = self.old_generate(input_embeds, input_atts, 100)
            # gf_loss, gf_future, recon_losses 
            output_dict = self.mtr_post_processing_test(output_embeddings, None, context_embs, batch_dict=batch_dict, other_features=other_features, subsample=False)
            return output_dict
        return outputs_dict

        # raise "Code not finished"
        # return
    def forward(self, input_ids, _training=True, **kwargs):
        if self.use_mtr:
            return self.mtr_forward(input_ids, _training)
        embed_tokens = self.llm.model.model.embed_tokens if _is_peft_model(self.llm) else self.llm.model.embed_tokens
        embed_device = embed_tokens.weight.device
    ...
    def load_mtr(self):
        ...
        intention_points = self.mtr_intention_points_file or os.environ.get(
            "IMOTION_LLM_MTR_INTENTION_POINTS",
            str(REPO_ROOT / "data" / "processed" / "waymo" / "mtr" / "output" / "cluster_64_center_dict.pkl"),
        )
        cfg.MODEL.MOTION_DECODER.INTENTION_POINTS_FILE = intention_points
        self.mtr = model_utils.MotionTransformer(config=cfg.MODEL)
        mtr_ckpt_path = self.mtr_ckpt_path or os.environ.get("IMOTION_LLM_MTR_CKPT")
        if not mtr_ckpt_path:
            raise FileNotFoundError(
                "MTR checkpoint path is not set. Use model.mtr_ckpt_path in the YAML or set IMOTION_LLM_MTR_CKPT."
            )
        self.mtr.load_params_from_file(filename=mtr_ckpt_path, logger=None)
        
        # raise "Model loading successfully"
        # return model
        return
    # @classmethod
    ...
    # def from_config(self, cfg):
        ...
        self.__init_additional_modules__(
            ...
            mtr = cfg.get("mtr", False),
            no_sys_prompt = cfg.get("no_sys_prompt", False),
            auto_txt_formatter = cfg.get("auto_txt_formatter", False),
            attention_input_adapter = cfg.get("attention_input_adapter", False),
            attention_input_adapter_numQ = cfg.get("attention_input_adapter_numQ", 114),
            mtr_cfg_path = cfg.get("mtr_cfg_path", None),
            mtr_ckpt_path = cfg.get("mtr_ckpt_path", None),
            mtr_intention_points_file = cfg.get("mtr_intention_points_file", None),
        )
        # model = cls(
        #     vit_model=vit_model,
        #     q_former_model=q_former_model,

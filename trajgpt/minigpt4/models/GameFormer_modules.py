    ...
    def forward(self, id, current_state, encoding, mask, act=None):
        ...
        if act is not None:
            if self.shared_act:
                ...
                # if valid_act_mask is None: # TODO: Fix this later if we need to pass action mask as input
                #     valid_act_mask = act != -1
                #     if sum(valid_act_mask)>0:
                #         act_masked = act[valid_act_mask].int()
                #         query_masked = query[valid_act_mask]
                #         query[valid_act_mask] = query_masked + self.act_embedding(act_masked)[:,None,:]
                # else:
                act_masked = act # assuming everything is valid, no masking
                query_masked = query
                query = query_masked + self.act_embedding(act_masked)[:,None,:]

...
class Decoder(nn.Module):
    def __init__(self, modalities, future_len, neighbors_to_predict, levels=3, act=False, shared_act=False, ego_act_only=False, num_act_classes=8):
...
class GameFormer_(nn.Module):
    def __init__(self, modalities, neighbors_to_predict, future_len, encoder_layers=6, decoder_levels=4, act=False, act_dec=False, shared_act=False, ego_act_only=True, no_fuse_act=False, num_act_classes =8):
        super(GameFormer_, self).__init__()
        self.encoder_01 = Encoder_01(neighbors_to_predict, encoder_layers, act)
        self.encoder_02 = Encoder_02(neighbors_to_predict, encoder_layers, act, shared_act, ego_act_only, no_fuse_act, num_act_classes)
        self.decoder = Decoder(modalities, future_len, neighbors_to_predict, decoder_levels, act_dec, shared_act, ego_act_only, num_act_classes)
...
class Encoder_02(nn.Module):
    def __init__(self, neighbors_to_predict, layers=6, act=False, shared_act=False, ego_act_only=False, no_fuse_act=False, num_act_classes=8):
        ...
        for i in range(N):
            ...
            if self.act and self.no_fuse_act:
                if i==0:
                    ...
                    masks[i] = torch.cat((masks[i], torch.zeros_like(inputs['act'][:,i]).bool()[:,None]), dim=1)
                else:
                    encoding = torch.cat((encoding, torch.zeros((inputs['act'].shape[0], 256), device=inputs['act'].device)[:,None]), dim=1)
                    masks[i] = torch.cat((masks[i], torch.ones_like(inputs['act'][:,i]).bool()[:,None]), dim=1)

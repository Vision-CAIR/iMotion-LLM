...
# Evaluation moved to base_task.py

#     @torch.no_grad()
#     def evaluation__(self, model, data_loader, epoch, cuda_enabled=True, num_eval_batches=None):
#         num_figs = 100
...
#         metric_logger.add_meter("worked", SmoothedValue(window_size=1, fmt="{value:.6f}"))
#         # metric_logger.add_meter("failed", SmoothedValue(window_size=1, fmt="{value:.1f}"))
#         failed=0
...
#                 if is_failed==1:
#                     failed+=1
#                     metric_logger.update(worked=0.0)
#                     # metric_logger.update(failed=1)
#                 else:
#                     worked+=1
#                     # rel_disc = torch.cat()
...
#                     metric_logger.update(worked=1.0)

#                     if wandb.run is not None:
#                     # images_to_log = {}
#                         if len(images_to_log)<=num_figs:
#                             img_name = f'ADE_FDE: {np.mean(ade.flatten().tolist()):.3f} _ {fde.mean().item():.3f}'
#                             images_to_log[img_name] = vizualize(samples['ego'][j].cpu(), samples['ground_truth'][j].cpu(), neighbors=samples['neighbors'][j].cpu(), map_lanes=samples['map_lanes'][j].cpu(), map_crosswalks=samples['map_crosswalks'][j].cpu(), region_dict=None, gt=True, fig=None)
...
#             logging.info("Averaged stats: " + str(metric_logger.global_avg()))
#             if wandb.run is not None:
#                 wandb.log({"ADE": metric_logger.ADE_mean.global_avg, "FDE": metric_logger.FDE_mean.global_avg, "Reconstruction ADE": metric_logger.recon_ADE_mean.global_avg, "Reconstruction FDE": metric_logger.recon_FDE_mean.global_avg,"Number Scenes": metric_logger.worked.global_avg*metric_logger.worked.count, "Failed Scenes": (1-metric_logger.worked.global_avg)*metric_logger.worked.count})

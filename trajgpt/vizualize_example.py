...
class DataProcess(object):
    ...
    def plt_scene(self, agent_1=None, agent_2=None, other_agents=None):
        ...
        if agent_1 is not None:            
            ...
            vizualize_agent_rect(fig=fig, past=agent_[0], object_type=agent_type, color=colors[agent_type], agent_prefix='0')
        ...
        if agent_2 is not None:            
            ...
            vizualize_agent_rect(fig=fig, past=agent_[0], object_type=agent_type, color=colors[agent_type], agent_prefix='1')
            vizualize_traj_arrow(agent_[0][-1,:3], agent_[1][:,:2], fig, ax, color=colors[agent_type], alpha=1, arrcolor='b', add_arrow=False) # single agent, single modality
    ...
    def onoffroad(self, traj, future_traj_):
        ...
        if not current_in and not final_in:
            inroad_state = {'current map state': 'off-road', 'future map state':'stay off-road', 'caution': 'be careful'}
            # print('an off-road car')
        if not current_in and final_in:
            inroad_state = {'current map state': 'off-road', 'future map state':'merge to the road', 'caution': ''}
            # print('an off-road car merging back onto the road')
        if current_in and not final_in:
            inroad_state = {'current map state': 'on-road', 'future map state':'go off-road', 'caution': 'be careful'}
            # print('going off-road car')
        else:
            inroad_state = {'current map state': 'on-road', 'future map state':'stay on-road', 'caution': ''}
        
        return inroad_state


    def gen_instruct_caption(self, history_ego, future_ego, history_interactive_neighbor, future_interactive_neighbor, other_neighbors):
        ...
        if True:
            ...
            # fig.savefig('ex.png')
            # plt.close()
            return fig, history_ego, history_interactive_neighbor
        ...
        if agent_type == 1: # Only if car
            state_features = {
                'Agent-A speed': str(np.linalg.norm(traj[-1,3:5]).round(2)),
                'Agent-A heading angle in radians': str(traj[-1,2].round(2)),
                'Agent-A location': "(0.00, 0.00)",
                }
            agent_name = 'A'
            ego_num = 0
            inter_num = 1
            onroad_state_ego = self.onoffroad(traj, future_traj_)
            instructs_ego = extract_instruct(future_traj_, traj)
            t_sceond_rule={}
            for ts in [1,3,8]:
            ...
            dict1 = {f"Agent-A {k}":v for k,v in onroad_state_ego.items()}
            dict2 = {f"Agent-A {k}":v for k,v in instructs_ego.items()}
            ...
            if len(inter_traj)!=0:
                ...
                if 'left' in category or 'right' in category:
                    ...
                    category = "on the left" if np.cross(my_vector, neighbor_vector)>0 else "on the right"
                rel_inter_2_ego = {"Agent-A view": f"The interactive neighbor, termed Agent-B, is {distance_:.2f}m away {category.lower()} of the ego Agent-A."}
            else:
                rel_inter_2_ego = {}

            # DONE: Stop signs & traffic light
            stop_sign_found = self.get_relevant_stop_sign(traj, detection_threshold=150)
            dict_stop = {f"Agent-A stop sign {k}":v for k,v in stop_sign_found.items()}
            traffic_sign_found = self.get_relevant_traffic_sign(traj, detection_threshold=150)
            dict_traffic = {f"Agent-A traffic sign {k}":v for k,v in traffic_sign_found.items()}
            # TODO: Close agents
            agents_within = self.get_agents_within(traj, inter_traj, other_neighbors, inter_num, 20)
            if len(agents_within)>0:
                agents_within_dict = {f"Agents within 20 meters from Agent-A": agents_within}
            else:
                agents_within_dict = {f"Agents within 20 meters from Agent-A": "No agents"}
            ...
            if agent_type_==1:
                ...
                state_features.update(
                    {
                    'Agent-B speed': str(np.linalg.norm(inter_traj[-1,3:5]).round(2)),
                    'Agent-B heading angle in radians': str(inter_traj[-1,2].round(2)),
                    'Agent-B location': f"({local_location[0]:.2f}, {local_location[1]:.2f})",
                    })
                ...
                if len(inter_traj)!=0:
                    ...
                    if 'left' in category or 'right' in category:
                        my_vector, _ = get_unit_vector(traj[-2,:2], traj[-1,:2])
                        category = "on the left" if np.cross(my_vector, neighbor_vector)>0 else "on the right"
                    rel_inter_2_ego.update({"Agent-B view":f"The ego agent, termed Agent-A, is {distance_:.2f}m away {category.lower()} of the interactive neighbor Agent-B."})
                ...
                stop_sign_found = self.get_relevant_stop_sign(traj, detection_threshold=150)
                dict_stop.update({f"Agent-B stop sign {k}":v for k,v in stop_sign_found.items()})
                traffic_sign_found = self.get_relevant_traffic_sign(traj, detection_threshold=150)
                dict_traffic.update({f"Agent-B traffic sign {k}":v for k,v in traffic_sign_found.items()})

                agents_within = self.get_agents_within(traj, inter_traj, other_neighbors, inter_num, 20)
                if len(agents_within)>0:
                    agents_within_dict.update({f"Agents within 20 meters from Agent-B": agents_within})
                else:
                    agents_within_dict.update({f"Agents within 20 meters from Agent-B": "No agents"})

                dict3 = {f"Agent-B {k}":v for k,v in onroad_state_inter.items()}
                dict4 = {f"Agent-B {k}":v for k,v in instructs_inter.items()}
            else:
                dict3 = {}
                dict4 = {}
    ...
    def process_data(self, viz=True,test=False, scenario_id='', sdc_ids_list=None, target_scene_folder=''):
        ...
        for data_file in data_files_list:
            ...
            for data in dataset:
                ...
                if scenario_id == '':
                    ...
                    if scenario_id != parsed_data.scenario_id:
                        continue
                # print(scenario_id)
                ...
                if sdc_ids_list is not None:
                    self.sdc_ids_list = [sdc_ids_list]
                else:
                    if test:
                ...
                for pairs in self.sdc_ids_list:
                    ...
                    out_ = self.gen_instruct_caption(ego[0], ground_truth[0], ego[1], ground_truth[1], neighbors)
                    if len(out_)>0:
                        fig, ego_h, inter_h = out_
                    else:
                        return {}

...
try:
    ...
    tf = None  # You can set it to None if you want to check this later in your code
import numpy as np
...
try:
    ...
    print("waymo_open_dataset is installed and available.")
except ImportError:
    print("waymo_open_dataset is not installed.")
    py_metrics_ops, config_util, motion_metrics_pb2  = None, None, None  # You can set it to None if you want to check this later in your code
...
class DrivingData(Dataset):
    def __init__(self, data_dir, act=False):
        ...
        self.act = act
        if act:
            self.data_list_act_dir = data_dir[:-2]+'_act'
...
def level_k_loss(outputs, ego_future, neighbor_future, levels, gmm=True, subsample=False):
    ...
    return loss, (future,best_mode,scores), ego

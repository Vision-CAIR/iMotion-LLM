...
def get_batch_instruct(ego_future, ego_state, num_classes=8):
    ...
    for batch_i in range(ego_future.shape[0]):
        if valid_mask[batch_i]:
            classify_track_out, turn_cls[batch_i], _, _ = ClassifyTrack(ego_future_cpu[batch_i], None, num_classes)
        else:
            turn_cls[batch_i] = -1
    return torch.cat((turn_cls, torch.ones_like(turn_cls)*-1), dim=-1)

def get_sample_instruct(ego_future, ego_state, num_classes=8):
...
def get_sample_instruct_c(ego_future, ego_state, num_classes=8):
    valid_mask = np.any(ego_future[:,0] !=0, axis=-1)
    if valid_mask:
...
def ClassifyTrack(track, valid_state_bit, num_classes=8):
    ...
    time_step = np.array([i*0.1 for i in range(len(track))])

...
valid_dir = 'training_30sep'
output_dir = root_dir+valid_dir+'_train_meta/train_meta.json'
...
if not two_agent:
        # if two_agent:
    dataset_ = TrajAlignDataset(root_dir+valid_dir+'/*', act=True, template_select=0, contrastive=False, train=False, two_agent=False, act_json=False, return_meta_data=True)

    # Calculate the number of samples (10% of the dataset)
    dataset_size = len(dataset_)
    indices = list(range(dataset_size))
    random.shuffle(indices)  # Shuffle the indices

    # Get 10% of the shuffled dataset
    ...
    loader = DataLoader(dataset_, batch_size=1, num_workers=0, sampler=sampler)
    ...
    if not os.path.exists(ff):
        for data in tqdm(loader):
            ...
            if len(list(data['positive_acts'][0].numpy()))>0:
    ...
    desired_per_label_count = [1633, 2529, 3173, 3573, 10058, 12424, 47, 514] # calculated based on the minimum for each direction
    for desired_label, desired_count in tqdm(enumerate(desired_per_label_count)):
        ...
        for k,v in meta_data_.items():
            ...
            if desired_label in v['neg1'] and c_neg1<desired_count and save_name not in meta_neg1.keys():
                ...
                c_neg1+=1
        print(f"Label {desired_label}: gt {c_gt1} -- pos {c_pos1} -- neg {c_neg1}")

...
# def calculate_angle_to_ref_point(ref_point, point):
#     direction_vector = point - ref_point
#     np.arctan2(direction_vector, )

# def calculate_angle(x, y):
#     # Calculate the dot product of vectors x and y
#     dot_product = np.dot(x, y)
#     # Calculate the magnitude (norm) of vector x and vector y
...
def on_map(future_state, map):
    ...
    # target_left_line_type = boundary_type_list[unrolled_map[nearest_idx[1]][11].int().item()]
    # target_right_line_type = boundary_type_list[unrolled_map[nearest_idx[2]][12].int().item()]
    assert torch.allclose(nearest_points[0], unrolled_map[nearest_idx[0]])
    target_self_line_type = self_line_type_list[nearest_points[0][10].int().item()]
    target_left_line_type = boundary_type_list[nearest_points[1][11].int().item()]
    target_right_line_type = boundary_type_list[nearest_points[2][12].int().item()]
...
def nearest_lines(curr_point, line):
    ...
    distance_to_nearest_points = [self_min_distance, left_min_distance, right_min_distance]
    ...
    if min(distance_to_nearest_points)<2:
        point_in_map = True

    if not point_in_map:
        print('NOT IN MAP')
    
    # TODO: Calcualte angle to each road edge that is withen 6 meters from the agents, then check if the agents is between any of two edges found.

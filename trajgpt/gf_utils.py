...
def imitation_loss(trajectories, ground_truth,gmm=True, subsample=True):
...
def gmm_loss(trajectories, convs, probs, ground_truth, subsample=True, reduction = 'mean', two_agent_reduction='yes'):
    # metric = [29, 49, 79]
    ...
    prob_loss = F.cross_entropy(input=probs, target=best_mode, label_smoothing=0.2, reduction=reduction)
    if reduction=='none':
        loss = loss + 2*prob_loss.unsqueeze(1)
        if two_agent_reduction!='none':
            loss = loss.mean(-1)
    else:
...
def level_k_loss(outputs, ego_future, neighbor_future, levels, gmm=True, subsample=True, reduction='mean', two_agent_reduction='yes'):
    if reduction=='none':
        if two_agent_reduction!='none':
            loss = torch.zeros_like(ego_future[:,0,0])
        else:
            loss = torch.zeros_like(torch.cat((ego_future[:,0:1,0], neighbor_future[:,0:1,0]), -1))
    else:
        loss: torch.tensor = 0
    ...
    for k in range(levels+1):
        ...
        if trajectories.shape[1]>1:
            ...
            if ego_future is not None:
                ...
                gt_future = ego_future
            neighbor=None
        if gmm:
            if gt_future is not None:
                convs = outputs[f'level_{k}_interactions'][..., 2:]
                gloss, best_mode, future, _ = gmm_loss(trajectories, convs, scores.sum(1), gt_future, subsample, reduction=reduction, two_agent_reduction=two_agent_reduction)
                loss += gloss
            else:
...
def motion_metrics(trajectories, ego_future, neighbor_future):
    ...
    neighborADE = distance.mean(-1)
    neighborFDE = distance[:, -1]

    # return egoADE.item(), egoFDE.item(), neighborADE.item(), neighborFDE.item()
    return egoADE, egoFDE, neighborADE, neighborFDE
...
#         self._prediction_trajectory.append(prediction_trajectory[..., :,:].clone().detach().cpu()) # self._prediction_trajectory.append(prediction_trajectory[..., 4::5,:].clone().detach().cpu())
